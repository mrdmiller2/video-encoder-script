#!/usr/bin/env bash
# convert-v6.0.4.sh — Organize movie folders, transcode TV/movies to AV1/x265 MKV.
# Cut forward from convert-v6.0.0D.sh (2026-09-14, D-val production-bridge
# stability review) purely to satisfy modules/ves-config.sh's own
# assert_script_name_matches_version() self-check -- VERSION was bumped to
# "6.0.4" on this branch (2026-09-10, long-shot scorer density commit) for
# the shared D-val/modules work, but no matching convert-v6.0.4.sh was ever
# cut, which left convert-v6.0.0D.sh permanently self-refusing to run under
# ITS OWN filename (confirmed live: every real invocation failed at startup
# with "Version 6.0.4 requires script filename convert-v6.0.4.sh"). No
# functional change from convert-v6.0.0D.sh -- do not overwrite this file in
# place on the next version bump; cut convert-v6.0.5.sh (or later) instead,
# per this project's own stated convention (README.md).
...
process_existing_av1() {
  ...
  local raw_decision
  if ! raw_decision="$(av1_source_reencode_sample_decision "$encode_src" "$ref_sz")"; then
  ...
  decision="$(printf '%s\n' "$raw_decision" | tr -d '\r' | grep -E '^(skip|av1|x265)$' | tail -n1 || true)"
  _set_sample_pred_from_output "$raw_decision" "$decision" "av1-source-recheck"
...
try_av1_convert() {
  ...
  # Chunk-parallel (Layer 1, opt-in via CONVERT_CHUNK_PARALLEL_ENABLED --
  # default off, not yet fleet-default per Phase 4 of the plan): hands
  # this source to the manifest/claim/encode machinery instead of a
  # whole-file encode. Deliberately does at most one chunk per call, same
  # as every other per-file job this loop runs -- the scan loop's own
  # repeated iteration is what drives a title's remaining chunks forward
  # across passes, exactly like ves-chunk-coordinator.sh's own header
  # comment on chunk_parallel_process_video describes.
  if chunk_should_split "$src"; then
    local chunk_rc=0
    chunk_parallel_process_video "$src" || chunk_rc=$?
    return "$chunk_rc"
  fi
  ...
  policy="$(size_keep_policy_av1 "$orig_sz" "$new_sz" "$upscaled" "$src")"
  local eff_upscale_lim=""
  [ "$upscaled" = true ] && eff_upscale_lim="$(effective_upscale_overshoot_pct "$orig_sz" "$src")"
...
try_x265_convert() {
  ...
  # Single entry point every AV1-to-x265 fallback funnels through (encode
  # failure, validation failure, size-overshoot -- all three call sites in
  # try_av1_convert land here) -- the natural place to guarantee the RAM
  # disk is genuinely clear of anything a prior codec attempt left behind
  # before this one starts competing for the same limited space. Also
  # correct (and a harmless no-op) for direct callers that never ran an
  # AV1 attempt at all (process_existing_x265, season_retry_pass).
  ...
  if [ "$src_codec" = "av1" ]; then
    ...
    policy="$(size_keep_policy "$orig_sz" "$new_sz" "$upscaled" "$src")"
  fi
  local eff_upscale_lim=""
  [ "$upscaled" = true ] && eff_upscale_lim="$(effective_upscale_overshoot_pct "$orig_sz" "$src")"
...
process_existing_x265() {
  ...
  local raw_decision
  if ! raw_decision="$(av1_source_reencode_sample_decision "$src" "$ref_sz")"; then
    warn "x265 sample test failed — leaving as-is: $src"
    record_skip "$src" "x265 sample test failed"
    return 0
  fi
  decision="$(printf '%s\n' "$raw_decision" | tr -d '\r' | grep -E '^(skip|av1|x265)$' | tail -n1 || true)"
  _set_sample_pred_from_output "$raw_decision" "$decision" "x265-source-recheck"
