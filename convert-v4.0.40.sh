#!/usr/bin/env bash
# convert-v4.0.40.sh — Organize movie folders, transcode TV/movies to AV1/x265 MKV.
# Version: 4.0.40
...
Install the tools or point to them explicitly:
  ...
  Environment variables:
    ...
    CONVERT_MKVALIDATOR path to mkvalidator (Matroska structure check; optional)
    CONVERT_MKVALIDATOR_ON_QUICK=1  run mkvalidator during scan (slow on large NAS trees)

  Command-line overrides:
    --ffmpeg PATH  --ffprobe PATH  --handbrake PATH
    --mkvpropedit PATH  --mkvmerge PATH  --mkvalidator PATH
  ...
  Platform: $PLATFORM
    Linux/WSL:  sudo apt install ffmpeg mkvtoolnix handbrake-cli python3 grep
                ...
                mkvalidator: build from https://www.matroska.org/downloads/mkvalidator.html
                  (install to ~/.local/bin — not in most distro repos)
    macOS:      auto re-execs under Homebrew bash 4+ (brew install bash ffmpeg mkvtoolnix handbrake python3)
...
# Short encode probe — Fedora HandBrake may report QSV while runtime init fails (dual-GPU Mac).
_probe_qsv_encode_available() {
  local tmp probe_out rc
  tmp="$(mktemp -d)"
...
discover_tools() {
  ...
  if tool="$(resolve_configured_tool "$TOOL_MKVALIDATOR" mkvalidator)" || tool="$(discover_binary mkvalidator)"; then
    ...
    warn "mkvalidator not found — using EBML segment-bounds check only (install to ~/.local/bin or set CONVERT_MKVALIDATOR)"
  fi
...
init_stats_log() {
  ...
  ts="$(date -u '+%Y-%m-%d %H:%M:%S UTC')"
  {
    ...
    echo "dry_run: $DRY_RUN | organize: $DO_ORGANIZE | convert: $DO_CONVERT | skip_av1: $SKIP_AV1 | skip_x265: $SKIP_X265 | nvidia: $HAS_NVIDIA | intel_qsv: $HAS_INTEL_QSV | amd_vce: $HAS_AMD_VCE | videotoolbox: $HAS_VIDEOTOOLBOX | active_encode: $ACTIVE_ENCODE_MODE | hw_decode: ${HW_DECODE_NAME:-none}"
...
validate_mkv_structure() {
  ...
  if [ "$quick" = true ] && [ "${MKVALIDATOR_ON_QUICK}" = "0" ]; then
    # Defer full mkvalidator to encode-time skip/validate (still safer than skipping).
    return 1
  fi
...
# Runs before structure/decode so we fail fast without a full container walk.
validate_mkv_metadata() {
  ...
  if [ -z "$codec" ] || [ "$codec" = "unknown" ]; then
    ...
    record_corrupt_mkv "$dst" "missing_video_stream"
    return 1
  fi
  return 0
}
...
convert_file_should_queue() {
  ...
  if has_complete_canonical_output "$f" "" true; then
    return 1
  fi
...
convert_library_batch() {
  ...
  for f in "${videos[@]}"; do
    ...
    if is_derived_output "$f"; then
      ...
      is_video_file "$f" || continue
      has_complete_canonical_output "$f" "" true && continue
      if should_skip_source_format "$f"; then
        local skip_reason
        skip_reason="$(skip_reason_for_format "$f")"
        log "Skip — $skip_reason: $f"
