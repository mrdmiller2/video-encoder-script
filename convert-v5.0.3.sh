#!/usr/bin/env bash
# convert-v5.0.3.sh — Organize movie folders, transcode TV/movies to AV1/x265 MKV.
# Version: 5.0.3
#
...
find_convert_videos_under() {
  ...
  local -a pred=() raw=()
  build_find_video_pred pred
  while IFS= read -r f; do [ -n "$f" ] && raw+=("$f"); done < <(find "$root" -type f "${pred[@]}" 2>/dev/null)
...
build_ffmpeg_video_args() {
  ...
  case "$codec" in
    av1)
      svtp="tune=0:enable-qm=1:qm-min=0:keyint=15s:scd=1:aq-mode=2"
      if [ "$anime" = true ]; then
  ...
  case "$codec" in
    av1)
      run_ffmpeg -y -v error -i "$clip" -c:v libsvtav1 -preset "$SVT_PRESET_SEARCH" -crf "$crf" \
        -pix_fmt yuv420p10le -svtav1-params "tune=0:enable-qm=1:qm-min=0" -an "$out" 2>/dev/null || return 1 ;;
    hevc)
...
ffmpeg_encode() {
  ...
  if [ "$acodec" = libopus ]; then
    args+=(-mapping_family 1 -af "aformat=channel_layouts=7.1|5.1|stereo|mono")
  fi
  args+=("${sub_args[@]}"
...
ffmpeg_encode_hw() {
  ...
  case "$enc" in
    av1_nvenc)  q=30; vargs=(-c:v av1_nvenc -preset p7 -tune hq -rc vbr -cq "$q" -b:v 0 -multipass fullres -spatial-aq 1 -temporal-aq 1 -highbitdepth 1) ;;
  ...
  local -a aextra=()
  if [ "$acodec" = libopus ]; then
    aextra=(-mapping_family 1 -af "aformat=channel_layouts=7.1|5.1|stereo|mono")
  fi
