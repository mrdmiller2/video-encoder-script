...
SKIP_AV1=false
SKIP_X265=false
# Override for movie/tv/anime auto-detection (--profile). Empty = auto
# (path-based detection). Needed for mixed-content library folders where the
# path alone can't tell the difference -- e.g. adult animation (South Park,
# Rick and Morty, ...) sitting inside a live-action /Television/American/
# folder, which would otherwise get the plain TV profile instead of anime's
# flat-color-appropriate tuning.
FORCE_PROFILE=""
SHARD_DEPTH=1
NO_SHARD=false
...
Options:
  ...
  --profile MODE          Force encoder profile to movie|tv|anime, overriding path-based
                          auto-detection for this whole run (e.g. adult animation sitting
                          inside a live-action /Television/ folder)
...
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    --profile)
      case "$2" in
        movie|tv|anime) FORCE_PROFILE="$2" ;;
        *) err "--profile must be movie, tv, or anime (got: $2)"; exit 1 ;;
      esac
      shift 2 ;;
...
# Classes: movie_sdr | movie_hdr | anime_sdr | anime_hdr | tv_sdr | tv_hdr
bakeoff_profile_key() {
  local src="$1"
  local base=movie
...
build_ffmpeg_video_args() {
  local codec="$1" crf="$2" src="$3" anime="$4" hdr="$5" hdr_mode="${6:-}"
...
fixed_crf_for() {  # codec anime tv hdr
  local codec="$1" anime="$2" tv="$3" hdr="$4"
  case "$codec" in
    av1)
...
resolve_crf_for_encode() {  # src codec anime hdr
  ...
  if [ "$hdr" = true ] || [ "$VMAF_DISABLED" = true ] || [ "$FF_HAS_LIBVMAF" != true ]; then
    crf="$(fixed_crf_for "$codec" "$anime" "$tv" "$hdr")"
    [ "$hdr" = true ] && log_err "HDR source — fixed CRF $crf (VMAF unreliable on PQ/HLG)" \
                      || log_err "Fixed CRF $crf ($codec$( [ "$anime" = true ] && printf ', anime')$( [ "$tv" = true ] && printf ', tv'))"
...
ffmpeg_encode() {
  ...
  local anime=false tv=false hdr=false hdr_mode crf rc acodec abr
  ...
  rc=0
  build_ffmpeg_video_args "$codec" "$crf" "$src" "$anime" "$hdr" "$hdr_mode" || rc=$?
  if [ "$rc" -eq 2 ]; then
  ...
  log "ffmpeg encode ($codec crf=$crf$( [ "$hdr" = true ] && printf ', HDR10')$( [ "$anime" = true ] && printf ', anime')$( [ "$tv" = true ] && printf ', tv')): $(basename "$src")"
  run_ffmpeg "${args[@]}"
  rc=$?
  if [ "$rc" -ne 0 ]; then
