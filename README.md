...
**Current release:** `convert-v5.1.0A.sh` (v5.1.0A) — see [What's new in v5](#whats-new-in-v5). Structural modularization (zero intended behavior change) — the script is now `convert-v5.1.0A.sh` plus a `modules/ves-*.sh` directory that must be deployed alongside it; see [CHANGELOG.md](CHANGELOG.md) for the full story. Deploying fleet-wide (safe alongside in-progress jobs — running encodes keep using their already-loaded script version; only the next-job pickup uses the new one).
...
| `convert-v5.1.0A.sh` | 5.1.0A | **Current** — structural modularization, zero intended behavior change: the 15,136-line monolith is now a 2,432-line orchestrator sourcing 30 files under `modules/ves-*.sh`, mirroring the Windows PowerShell port's already-proven module boundaries. Requires deploying `modules/` alongside the script — the multi-file deploy mechanism (Phase 0 of this work) was extended to push both atomically. See [CHANGELOG.md](CHANGELOG.md) for the full story. |
