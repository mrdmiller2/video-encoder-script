...
**Current release:** `convert-v5.1.0F.sh` (v5.1.0F) — see [What's new in v5](#whats-new-in-v5). Structural modularization (v5.1.0A, zero intended behavior change) — the script is now `convert-v5.1.0F.sh` plus a `modules/ves-*.sh` directory that must be deployed alongside it — plus real bug fixes found via post-modularization regression testing (multi-part movies never being merged, v5.1.0B; a heartbeat-subshell leak on interrupt, v5.1.0C), a new quality-review feature (v5.1.0D): a kept output whose final measured VMAF lands below 85.00 is now flagged for human review on both ports, and two team-review follow-up passes (v5.1.0E, v5.1.0F) that found and fixed several real bugs — most severely, the Windows port's main scan loop was re-encoding its own prior outputs on every rescan (now fixed on both platforms for cross-OS shared-library safety). See [CHANGELOG.md](CHANGELOG.md) for the full story. Deploying fleet-wide (safe alongside in-progress jobs — running encodes keep using their already-loaded script version; only the next-job pickup uses the new one).
