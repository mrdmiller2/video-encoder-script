...
| `convert-v5.1.0S.sh` | 5.1.0S | **Current** — Fixed a false-positive "below VMAF floor" flag on VFR sources: `measure_final_vmaf()`/`Get-VesFinalVmaf` compared misaligned frames when the source had duplicate/dropped frames drifting its timing from the CFR output, silently scoring near 0 and dragging the pooled mean down (dozens of episodes across 5 shows/5 machines wrongly flagged — the actual encodes were fine). Fixed by normalizing both streams to the source's frame rate before comparison; verified 53.0 → 92.2 on the same real file. See [CHANGELOG.md](CHANGELOG.md) for the full story. |
