...
| `convert-v5.1.1H.sh` | 5.1.1H | **Current** — Two real bugs found root-causing a genuine PRINCE production failure during the full-history regression test ("A Clockwork Orange (1971)" failed after ~13 hours): RAM-disk space exhaustion silently corrupted an in-progress x265 stage-1 encode (caught correctly by existing post-encode validation, no bad output kept, but real compute wasted), and separately, the AV1 attempt's own crash diagnostics were unrecoverable because both AV1 and x265 fallback attempts reused an identical PID-only log filename. Fixed both, both platforms: diagnostic stderr logs now write to genuine local disk, never the RAM disk (reserved exclusively for encode data); codec is now part of every diagnostic log filename so a fallback retry can't clobber the failed attempt's evidence. Syntax-verified only, not yet exercised live — holding off on relaunching per explicit user direction while the RAM-disk-exhaustion root cause is investigated further. See [CHANGELOG.md](CHANGELOG.md) for the full story. |
