...
| `convert-v5.1.0X.sh` | 5.1.0X | **Current** — Fleet-validation triage pass, both platforms: (1) fixed a real hang in Windows' orphan reaper — `Get-ChildItem -Recurse -Filter` silently ignores `-Filter` when the target is a single file, so single-file `-SearchPath` invocations were feeding the multi-GB source video itself to `Get-Content` as a "flag candidate," hanging for 20+ minutes reading it as text (invisible until today's SMB ACL fix let the read actually proceed instead of failing fast); (2) root-caused the fleet's shared "Permission denied"/`icacls` ACL-widen failures on freshly-created `ffmpeg-logs/` sidecar dirs to Samba's default `create mask`/`directory mask` on all 3 Media shares — fixed server-side via TrueNAS `auxsmbconf` (`create/directory mask = 0777`, `force create/directory mode = 0777`), verified live; (3) removed `enable-hdr=1` from both platforms' svtav1-params — never a real SVT-AV1 option on any version checked, silently logged-and-ignored on every HDR encode this project has ever run, first noticed when SVT-AV1 4.2.0 (fleet standard is v4.1.0) logged the parse failure loudly. See [CHANGELOG.md](CHANGELOG.md) for the full story. |
