...
| `convert-v5.1.1A.sh` | 5.1.1A | **Current** — Diagnostic safeguard for the frame-duplication defect found auditing a 38-file fleet-wide below-VMAF-floor backlog (all 38 re-verified genuine, zero false positives; one file deep-investigated found real duplicate packets baked into the AV1 output — invisible to a still-frame comparison but catastrophic to VMAF). Root cause not conclusively pinned down (tool binaries and encode command both unchanged since the defective batch; isolated reproduction is clean; cross-platform/cross-VM pattern in one narrow time window points to fleet-wide concurrent NAS load as the likely trigger, not provable from surviving logs). New `detect_output_frame_duplication()`/`Get-VesOutputFrameDuplication` (reusing the existing DTS-delta-variance technique against the output instead of the source) turns a generic "NEEDS REVIEW" flag into a self-diagnosing one. Ported to Windows in full, including the previously-never-ported `_dts_delta_cv` primitive; code-complete and syntax-validated but live Windows functional verification still pending (all 3 Windows machines were mid-job). Also requeued all 38 confirmed-defective files for re-encoding fleet-wide and fixed an unrelated real deployment bug found along the way (a prior RAM-disk fix's config file silently never landed on 5 of 6 Linux/macOS machines). See [CHANGELOG.md](CHANGELOG.md) for the full story. |
