...
**Current release:** `convert-v5.0.32T.sh` (v5.0.32T) — see [What's new in v5](#whats-new-in-v5)
...
| `convert-v5.0.32T.sh` | 5.0.32T | **Current** — fixed a real bug found during a full fleet log audit: `process_video()` discarded the real success/failure return code from `process_existing_av1`/`process_existing_x265`, so a genuine encode/validation failure (source needed repair, output also failed validation, correctly discarded) silently logged as "Job complete" instead of "Job failed" — no data lost, but invisible to monitoring and, separately, ineligible for retry on resume. Independently reviewed, all approved. See [CHANGELOG.md](CHANGELOG.md). |
