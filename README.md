...
| `convert-v5.1.1E.sh` | 5.1.1E | **Current** — Routed live ffmpeg diagnostic stderr off the NAS onto local/RAM-disk staging (both platforms), found while investigating a user report that RAM-disk utilization looked low during a 9-machine weighted stress test. Empirically confirmed the per-title stderr log grows continuously throughout an encode (~every 0.5s, for however long a real encode runs) and was resolving to the NAS folder next to the source by default — real, avoidable NFS write traffic for a diagnostic stream nobody reads while the job is healthy. Fixed by routing the live tee to the same local/ramdisk directory the encode's own binary output already stages to, copying the finished log back to the NAS sidecar only if non-empty. Scope per explicit user direction: only multi-connection-blocking (title-locks), progress-tracking (resume-state/done-log, the Windows `-progress` file), and human-inspection (`bad_sources.txt`, `corrupt_files.txt`, etc.) files stay on the NAS — everything else "live" during the encode now stays local. Not yet exercised by a real fleet encode (syntax-verified only; the concurrent 9-machine stress test was already running v5.1.1D and was left alone). See [CHANGELOG.md](CHANGELOG.md) for the full story. |
