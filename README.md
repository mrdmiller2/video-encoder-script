...
**Current release:** `convert-v5.0.33H.sh` (v5.0.33H) — see [What's new in v5](#whats-new-in-v5)
...
| `convert-v5.0.33H.sh` | 5.0.33H | **Current** — full end-to-end team review of the entire file (7 parallel review passes plus a consolidated pass over all changed functions together). Nine real bugs fixed: a resume-bookkeeping abort risk, 2 bare `rm -rf`/`rm -f` abort risks, 4 bare `mktemp -d` abort risks (all under `set -e`, all now match their own function's existing fallback convention), an orphan-recovery gap for the new bare-`.mkv` remux output (found dead-on-arrival on the final verification pass because the staged-file enumerator never listed that filename shape at all — fixed alongside a too-narrow extension-reconstruction list), and a subtitle-probe ambiguity gap that treated any non-timeout ffprobe error as "confirmed no subtitles." One reviewer reported zero bugs; another reviewer and one section agent independently found real, empirically-verified issues, so those were trusted over that clean bill of health. Three architectural items deferred to ROADMAP.md rather than spot-fixed. See [CHANGELOG.md](CHANGELOG.md) for the full story. |
