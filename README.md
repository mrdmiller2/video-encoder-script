...
**Current release:** `convert-v5.1.0I.sh` (v5.1.0I) — see [What's new in v5](#whats-new-in-v5). Structural modularization (v5.1.0A, zero intended behavior change) — the script is now `convert-v5.1.0I.sh` plus a `modules/ves-*.sh` directory that must be deployed alongside it — plus real bug fixes found via post-modularization regression testing (multi-part movies never being merged, v5.1.0B; a heartbeat-subshell leak on interrupt, v5.1.0C), a new quality-review feature (v5.1.0D): a kept output whose final measured VMAF lands below 85.00 is now flagged for human review on both ports, four team-review follow-up passes (v5.1.0E–H) closing real gaps toward full Windows/bash feature parity — most substantially, Windows gained a real size-guard + x265 fallback (previously kept any AV1 regardless of size, and v5.1.0H fixed a real data-loss bug in that same fallback's `-InPlace` interaction) and both platforms now cross-recognize each other's title-locks and completed outputs on a shared library — and a v5.1.0I fix for a real bug found resuming a live fleet test: `-p` targeting a real file whose name contains literal `[`/`]`/`*`/`?` characters (e.g. a release-group tag) was being silently misread as a directory+glob instead of the literal file. See [CHANGELOG.md](CHANGELOG.md) for the full story. Deploying fleet-wide (safe alongside in-progress jobs — running encodes keep using their already-loaded script version; only the next-job pickup uses the new one).
