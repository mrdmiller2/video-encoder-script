...
**Current release:** `convert-v5.1.1N.sh` (v5.1.1N) — fixed a real remux-stage RAM-disk-exhaustion gap, found by a fleet health-check audit independent of the PRINCE crash investigation: ELVIS's "Mad Heidi (2022)" AV1 attempt failed at the remux stage with `rc=-28` "There is not enough space on the disk," despite the v5.1.1J two-tier sizing fix already being deployed, because that fix only ever addressed the encoder's own internal memory need — a different resource than the RAM-disk file-space needed to hold stage 1's output and the final remuxed output *simultaneously* during remux (roughly 2x a single file's worth of space), which `resolve_encode_stage_path()`/`Resolve-VesEncodeStagePath` never accounted for (it only ever sizes against the source file, once, before stage 1 starts). Fixed on both platforms: right after stage 1 succeeds, a new check compares its real on-disk size against actual current free RAM-disk space, and retargets only the final remux output to local disk if there isn't room for a second same-size file — stage 1 stays put untouched. See [CHANGELOG.md](CHANGELOG.md) for the full story. Older summary below (v5.1.1M): temporary diagnostic instrumentation added while root-causing the "Happiness for Beginners (2023)" `ffmpeg.exe` access-violation crash on PRINCE: after both the v5.1.1K and v5.1.1L logging fixes, a 4th crash-reproduction attempt still produced no stderr sidecar file at all, with no warning from either fix path. `Invoke-VesTrackedProcess` now writes `[capture started ...]`/`[capture ended, exitcode=...]` marker lines around the capture, and hardens the stdout/stderr reads against a faulted async Task, so the next crash will definitively show whether the file is never created, created-then-lost, or genuinely empty. Intended to be removed once the crash is root-caused. See [CHANGELOG.md](CHANGELOG.md) for the full story. Older summary below (v5.1.1L): fixed a second logging bug found immediately while verifying v5.1.1K live: a third PRINCE retest of "Happiness for Beginners (2023)" reproduced the same `ffmpeg.exe` access violation, but the new v5.1.1K stderr capture was *still* missing afterward — traced to the v5.1.1I RAM-disk stale-attempt-file sweep, which silently `rm`/`Remove-Item`'d the file whenever its move-to-holding-dir failed (a just-crashed process's file is plausibly still transiently locked exactly when the sweep runs), on both platforms, despite its own doc comment promising to preserve files for a human to look at. Fixed with a retry-then-copy-then-delete fallback plus an explicit warning if both fail, so evidence is never silently destroyed again. See [CHANGELOG.md](CHANGELOG.md) for the full story. Older summary below (v5.1.1K): fixed a Windows-only diagnostic-logging gap found while retesting v5.1.1J live on PRINCE: relaunching "Happiness for Beginners (2023)" after the v5.1.1J deploy reproduced the exact same crash (`ffmpeg.exe` access violation, `0xc0000005`, identical fault offset both times) but with 16.6GB of 31.7GB RAM still free — ruling out memory pressure and pointing at a genuine deterministic native crash in `ffmpeg.exe` itself, confirmed via the Windows Event Log's Application Error record. Digging further was blocked by a real gap: `Invoke-VesTrackedProcess` captured stderr entirely in memory via `.NET`'s `ReadToEndAsync()` and only wrote it to disk after `WaitForExit()` returned, so a hard crash — which tears down stdio pipes without a clean exit — left the sidecar log completely empty both times, with no way to see what the encoder printed before dying. Fixed by rewriting the capture as a synchronous polling read loop (`ReadLineAsync()` with a bounded wait, checked against `$proc.HasExited`) that appends and flushes every stderr line to disk as it arrives, so a mid-encode crash now leaves a real partial trail instead of nothing. Windows-only fix — bash's stderr capture already streamed live via `tee`; the bash version bump is for fleet parity only. Syntax-verified; a live crash-reproduction re-test on PRINCE is in progress to capture the actual root-cause diagnostic, and the same source is being tried on a second machine to check whether this is file-specific or PRINCE-specific. See [CHANGELOG.md](CHANGELOG.md) for the full story. Older summary below (v5.1.1J): root-caused why the RAM-disk-exhaustion failures kept happening: a real PRINCE crash (`SvtMalloc[fatal]: allocate memory failed` on a true 4K/10-bit source) turned out to be a genuinely distinct issue from the earlier `ENOSPC` failures — not the RAM disk running out of file-write space, but the encoder's own internal memory (SVT-AV1's picture-buffer pool, ~7.6GB at that resolution/GOP alone) getting squeezed because the RAM disk's flat 60%-of-available formula was claiming the majority of free memory before the encoder even started. Fixed on both platforms with two-tier sizing per explicit user direction: below 64GB available, the RAM disk is now capped at a flat 12GB regardless of how much more is technically free, leaving generous headroom for the encoder; at or above 64GB available, 45% of available is used (down from 60%). All thresholds are named, overridable config constants, not hard-coded. Verified against 5 realistic scenarios (28GB/60GB/64GB/90GB/8GB available) confirming correct tier selection and the safety clamp for very-constrained machines. Doesn't eliminate the possibility of a genuinely oversized encode still exceeding available memory (an inherent limit) but removes the RAM disk as a major avoidable contributor. Syntax-verified only, not yet exercised live. See [CHANGELOG.md](CHANGELOG.md) for the full story. Older summary below (v5.1.1I): explicit user hypothesis, confirmed and fixed: when the pipeline falls back from a failed/discarded AV1 attempt to a real x265 encode, anything the AV1 attempt left behind in the RAM disk was competing with the x265 attempt for the same limited space — directly contributing to the RAM-disk-exhaustion failure v5.1.1H already root-caused. The RAM-disk staging directory is reused across a whole job's lifetime (both codec attempts stage into it, only torn down at job end), so a crashed AV1 encode whose own cleanup didn't run could leave partial data sitting there while x265 started fresh alongside it. Fixed on both platforms: before any x265 fallback begins, the RAM disk staging area is now explicitly swept — anything found gets moved to a local-disk holding directory (not deleted, in case it's worth a look for debugging a repeat crash). Also found during this session's audit: ELVIS's "Mad Heidi" run was previously misreported as a clean success — it actually failed with the identical RAM-disk `ENOSPC` signature as A Clockwork Orange, traced to a stale unrelated log entry being read by mistake; needs a re-run. Not fixed this release (separate, already-flagged): a systemic Windows-only gap where "copy the diagnostic log back only if non-empty" doesn't actually filter anything, because Windows's stderr-capture method always returns a full blob at exit. Syntax-verified only, not yet exercised live. See [CHANGELOG.md](CHANGELOG.md) for the full story. Older summary below (v5.1.1H): two real bugs found root-causing a genuine PRINCE production failure during the full-history regression test: "A Clockwork Orange (1971)" failed after ~13 hours. Traced to RAM-disk space exhaustion corrupting an in-progress x265 stage-1 encode (silently stopped partway but exited with a clean success marker — correctly caught by the existing post-encode duration-mismatch validation, so no bad output was kept, but real compute was wasted). While root-causing it, found a second bug: the AV1 attempt's own crash diagnostics were unrecoverable because both the AV1 and x265 fallback attempts reused an identical PID-only log filename, so the x265 attempt's log silently overwrote the AV1 crash's evidence. Fixed both, on both platforms, per explicit user direction: (1) diagnostic stderr logs now write to genuine local disk, never the RAM disk — the RAM disk is reserved exclusively for encode data, no longer sharing that scarce space with logs; (2) codec is now part of every diagnostic log filename (`<title>.<codec>.<pid>.stderr.log`), so a codec-fallback retry can no longer clobber the failed attempt's own evidence. Syntax-verified only — not yet exercised by a real fleet encode, per explicit user direction to hold off on relaunching while the RAM-disk-exhaustion root cause itself is investigated further (separate future work; this release only stops logs from competing for that space). See [CHANGELOG.md](CHANGELOG.md) for the full story. Older summary below (v5.1.1G): closed a real Windows/bash feature-parity gap, user-flagged as high severity: the proactive per-source VFR/CFR detection + baseline self-VMAF check (`detect_frame_rate_mode()`/`measure_source_baseline_vmaf()`, bash's v5.1.0W) had no Windows port at all — confirmed via an exhaustive grep of every `.psm1` module on a live fleet machine, zero hits. Not a quality-safety hole in the VMAF comparison math itself (that fix, v5.1.0S/T's measure-both-ways-take-max frame-rate normalization, was already correctly ported in `Get-VesFinalVmaf`) — what was missing was the proactive detection/logging/ambiguous-flagging layer that runs before CRF search. Fixed by porting `Get-VesSourceFrameRateMode`, `Get-VesSourceBaselineVmaf`, and `Write-VesSourceTraitsAmbiguousFlag`, reusing already-ported helpers (`Get-VesComplexitySamplePoints`, `Get-VesDtsDeltaCv`, `Get-VesFinalVmaf`) rather than duplicating logic, wired into `convert.ps1` at the same point bash uses the equivalent pair. Verified live on RANDYJ against real data: numerically identical result to bash on the same file (`cfr avg_cv=0.0109`). Related smaller finding, not fixed this release: Windows's `-DryRun` bails out much earlier in the pipeline than bash's `--dry-run` does. No bash changes this release — version bumped anyway to keep bash/Windows on one shared version number. See [CHANGELOG.md](CHANGELOG.md) for the full story. Older summary below (v5.1.1F): second pass of the same NAS-load/RAM-disk audit, per explicit user direction to triage "even small ones" before restarting the weighted stress test. Real bug found: `qtgmc_deinterlace_to_intermediate()`'s multi-GB lossless intermediate used `CONVERT_RAMDISK_DIR` (a rarely-set user override, empty by default) instead of `RAMDISK_JOB_DIR` (the actual runtime-resolved per-job ramdisk) — fell through to plain `/tmp` on every real run, harmless on Linux (tmpfs) but real local disk on macOS. Fixed to use `RAMDISK_JOB_DIR` directly; also updated every CRF-search sample-encode temp file in `modules/ves-vmaf-crf-search.sh` to prefer it too, for consistency. Everything else audited (VMAF comparison, the title-lock heartbeat, HandBrake temp clips, all Windows temp-file usage) was already correct. See [CHANGELOG.md](CHANGELOG.md) for the full story. Older summary below (v5.1.1E): routed live ffmpeg diagnostic stderr off the NAS onto local/RAM-disk staging on both platforms, found while investigating a user report that RAM-disk utilization looked low during a 9-machine weighted stress test. Direct empirical test confirmed the per-title stderr log grows continuously throughout an encode (~every 0.5s, for however many hours a real encode runs) and was resolving to the NAS folder next to the source, not a local path — a real, avoidable NFS write for a diagnostic stream nobody reads while the job is healthy. Fixed by routing the live tee to the same local/ramdisk directory the encode's own binary output already stages to, copying the finished log back to the NAS sidecar only if non-empty (a real warning worth a human look). Scope, per explicit user direction: only files that block multi-connections (title-locks), track progress (resume-state/done-log, the Windows `-progress` file), or exist for human inspection (`bad_sources.txt`, `corrupt_files.txt`, etc.) stay on the NAS — everything else "live" during the encode now stays local. See [CHANGELOG.md](CHANGELOG.md) for the full story. Older summary below (v5.1.1D): fixed a validation-retry bug found during a 20-file movie-length fleet stress test: the shared timeout-wrapped-retry helper (`_run_timeout_retry`/`Invoke-VesWithTimeoutRetry`) retried a timed-out `mkvalidator`/`ffprobe`/`mkvmerge` call using the *same* timeout budget every attempt, so a large source that genuinely needed more time than a single (already well-tuned) size-scaled budget allowed got permanently skipped as "possibly stalled" even though nothing was actually stalled — all 3 attempts (1 initial + 2 retries) failed identically since retrying with an unchanged budget can never help. Fixed by doubling the timeout on each retry instead of reusing it, on both platforms. Also investigated the same session and found (not fixed, a separate scoped task): Windows encodes never get an embedded quality tag on passing files because no MKVToolNix (`mkvpropedit`) is provisioned on the Windows fleet at all — the safety-critical below-floor flagging is unaffected. See [CHANGELOG.md](CHANGELOG.md) for the full story. Older summary below (v5.1.1C): fixed the real-encode VMAF muxer-timestamp measurement bug (task #172), found at scale when a fleet-wide library re-scan turned up an entire 52-episode series (Falling Skies) landing well below the VMAF floor despite healthy CRF-search predictions. Root cause: ffmpeg's MKV muxer relabels the output's PTS with an offset vs the source, so two independent same-nominal-timestamp seeks into source/output can land on genuinely different real-content frames. Two earlier fix attempts (an unconditional measured-offset correction, then a max-of-2-candidates version) were each disproven by real tests before landing on the shipped fix: search every whole-frame offset in a small range per sample window and keep whichever scores highest — misalignment can only ever drag a score down, never inflate it, so an exhaustive local search is safe in every direction. Costs ~5-8 minutes per file (accepted deliberately — a once-per-file diagnostic tag, not a hot path). See [CHANGELOG.md](CHANGELOG.md) for the full story, including the real regression the naive fix caused before it was caught. Older summary below (v5.1.1B): root cause found and fixed for the v5.1.1A frame-duplication defect: `-fps_mode passthrough` on the stage-1 encode was blindly propagating a decoded-PTS irregularity in certain sources' opening ~30 seconds into a persistent duplicate-frame pattern for the rest of the job (100% reproducible fleet-wide on Stargate Universe S01/Marvel's Runaways S02, isolated via direct clip reproduction, fixed by switching to `-fps_mode cfr`). See [What's new in v5](#whats-new-in-v5) and the version table below for v5.1.0X/Y/Z/v5.1.1A. Older summary below (v5.1.0W): v5.1.0W adds proactive per-source VFR/CFR detection + baseline self-VMAF (`modules/ves-source-traits.sh`) before CRF search, unconditional across every profile — a proactive counterpart to the VMAF-VFR bug fixed in v5.1.0S/T, which was only ever caught reactively. Also this session: a fleet-wide re-verification pass found and auto-corrected ~130 files fleet-wide mistagged "NEEDS REVIEW" by the pre-fix VMAF measurement (e.g. Snowpiercer 53.0→92.2, Godfather of Harlem 66.9→99.4); root-caused and fixed the NAS's root-squash bug that caused every "Permission denied" writing shared sidecar dirs (fixed via TrueNAS API, `maproot_user=root`); and made real but incomplete progress on the separate Windows-side SMB `icacls` ACL-widen failures — traced through NFSv4 ACL gaps and a silent guest-auth fallback (both fixed) down to a remaining Samba ACL-translation gap not yet resolved. Full story in [CHANGELOG.md](CHANGELOG.md). v5.1.0V generalized v5.1.0U's fix per explicit direction — every file this pipeline writes should match its source path's own permissions/ownership, not a hard-coded 0777 guess. New `match_source_permissions()` derives mode/owner/group from a real reference path and applies them; wired into `finalize_mkv_output` (the single choke point every finished output goes through fleet-wide) and into `ensure_shared_sidecar_dir` (now derives from the parent dir instead of hard-coding 777). The chmod half works reliably (verified 666→777 live); the chown half is confirmed blocked server-side by this NAS export for every client including root (a local, non-NAS chown with the same command succeeds instantly) — not fixable client-side, kept as harmless best-effort. v5.1.0U fixed real "Permission denied" failures on bash writing shared NAS sidecar directories (`ffmpeg-logs/`, the validation-failure evidence dir, `Deferred/`): a freshly-created directory doesn't inherit this library's 777/admin:8080 convention, and on this NAS resolves to mode 0775 owned by an unmapped "nobody:8080" instead — since `worker` on every machine is in neither, later writers got locked out entirely (found live: AI-PROCESSOR and LAYTOYAJ both hit this during a real batch). Bash never had the self-heal the Windows port already has (`Set-VesEveryoneReadWrite`); added `ensure_shared_sidecar_dir()`, which widens the mode and falls back to passwordless `sudo -n chmod` when a plain chmod fails (confirmed live: a non-owner's plain chmod is rejected outright, sudo fixes it, root is not squashed on this NAS). Verified end-to-end against the real broken directory. v5.1.0T corrected v5.1.0S's VMAF frame-rate fix (it wasn't safe to apply unconditionally — some vintage sources have real subtle timing irregularities despite avg/nominal frame rates matching exactly, and forcing the fps filter made those worse; now measures each window both ways and keeps the higher score, safe in both directions) and fixed a severe uncaught-exception bug on the Windows port that destroyed 100% of a 36-episode overnight batch on RANDYJ (a diagnostic stderr-log write failure was propagating up and discarding already-completed real encodes — now a non-fatal warning, matching the surrounding code's own stated intent). v5.1.0S fixed a false-positive "below VMAF floor" bug found sweeping fleet logs for issues: `measure_final_vmaf()`/`Get-VesFinalVmaf` independently seek into the original source and the finished output for a same-timestamp libvmaf comparison, which silently mis-scores near 0 on a VFR source (duplicate/dropped frames drift the two files' per-frame timing apart) — dozens of episodes across five shows on five machines had been flagged as quality-degraded when the actual encode was fine all along (verified via a real reproduction: a flagged Snowpiercer episode's still frames looked visually identical, and its per-frame VMAF log showed a literal ~3-frame-period 90-100/0.0 alternating pattern). Fixed by normalizing both streams to the source's nominal frame rate before comparison on both platforms; verified end-to-end, 53.0 → 92.2 on the same file. v5.1.0R fixed two more real gaps found monitoring live fleet batches: the subtitle text-decode probe now short-circuits as soon as real content is confirmed (both platforms — was running to full completion every time, causing PRINCE to repeatedly burn its whole retry budget across many Sabrina episodes; verified real speedup, 2+ min → 43.5s on a real file), and `season_retry_pass()` now self-verifies it processed every promised retry candidate after JJACKSON silently completed only 1 of 3 promised season-retry episodes with zero trace (root cause not pinned down despite an isolated repro behaving correctly; now loud instead of silent if it recurs). v5.1.0Q fixed a real gap found monitoring a live fleet batch: `remux_copy_to_mkv()` (the shortcut path for sources that already have the desired codec and only need a container change — shared by the must-eliminate-format floor, HEVC-in-MKV shortcut, and legacy-container remux) had no fallback for a malformed-but-non-empty subtitle track, so one bad `mov_text` track failed the whole remux (losing valid video+audio too) instead of just dropping the bad subtitle — fixed with the same retry-without-subtitles fallback the main two-stage encode path already had, verified against a real failing source (5 "Marvel's Jessica Jones" S03 episodes on Plex). v5.1.0M added real field-based deinterlace (QTGMC via VapourSynth) for the `vintage` profile on both platforms; v5.1.0N fixed four real bugs found via a genuine single-file production test; v5.1.0O fixed five more found by multi-tool review of v5.1.0N's own fix set (most seriously a Windows cross-function scoping bug that made two of those fixes complete no-ops); v5.1.0P fixed four more found by a further review round on v5.1.0O's own timeout fix — the `--foreground` flag it relied on (via the shared `run_with_timeout` wrapper) turns out not to create a new process group, so it never actually protected against a hung QTGMC transcode either, plus a `set -e` risk, a VMAF-cache entry-clear regression, and a real live gap on the fleet's one macOS machine (no `timeout`/`gtimeout`/`setsid` at all) — verified across four consecutive real production encodes and a final clean review round. See [CHANGELOG.md](CHANGELOG.md) for the full story on all four rounds. Structural modularization (v5.1.0A, zero intended behavior change) — the script is now `convert-v5.1.0L.sh` plus a `modules/ves-*.sh` directory that must be deployed alongside it — plus real bug fixes found via post-modularization regression testing (multi-part movies never being merged, v5.1.0B; a heartbeat-subshell leak on interrupt, v5.1.0C), a new quality-review feature (v5.1.0D): a kept output whose final measured VMAF lands below 85.00 is now flagged for human review on both ports, four team-review follow-up passes (v5.1.0E–H) closing real gaps toward full Windows/bash feature parity — most substantially, Windows gained a real size-guard + x265 fallback (previously kept any AV1 regardless of size, and v5.1.0H fixed a real data-loss bug in that same fallback's `-InPlace` interaction) and both platforms now cross-recognize each other's title-locks and completed outputs on a shared library — a v5.1.0I fix for a real bug found resuming a live fleet test: `-p` targeting a real file whose name contains literal `[`/`]`/`*`/`?` characters (e.g. a release-group tag) was being silently misread as a directory+glob instead of the literal file — a v5.1.0J fix for a NAS/SMB-specific reliability bug in the Windows port's human-review flag writers (plus matching NFS multi-host append hardening on bash) — a v5.1.0K RAM-disk audit: none of the 3 live Windows fleet machines had ever actually been using RAM-backed staging (opt-in switch never passed), RAM disk sizing raised from 40% to 50% of available memory fleet-wide, and a real macOS available-memory undercounting bug fixed (was starving Crystalight's RAM disk to 1.5GB on a 64GB machine) — and a v5.1.0L fix for a real silent-failure bug found in a live 15-episode PRINCE batch ("12 ok, 3 failed" with zero explanation): the size-guard/x265-fallback logic was silently discarding fully-completed encode work with no trace, now fixed with explicit logging routed through the existing human-review mechanism, plus a matching directory-level NAS ACL fix for `ffmpeg-logs` stderr sidecars. See [CHANGELOG.md](CHANGELOG.md) for the full story. Deploying fleet-wide (safe alongside in-progress jobs — running encodes keep using their already-loaded script version; only the next-job pickup uses the new one).
...
| `convert-v5.1.0C.sh` | 5.1.0C | Fixes a real, pre-existing bug: `run_tracked_encoder()`'s periodic in-progress-flag heartbeat subshell could survive for up to 300s after a SIGINT/SIGTERM interrupt, since the trap handler unwound the script before the subshell's own cleanup ever ran. Took three iterations to actually fix (each verified via real interrupt tests) — a plain `kill` doesn't reach the subshell's currently-running `sleep` child, and a naive `pkill -P` issued after the parent is already dead loses the race against kernel reparenting. Final fix snapshots the child PID before killing the parent; team review then caught a `set -e` interaction that could have aborted the whole interrupt-cleanup sequence, fixed in the same pass. See [CHANGELOG.md](CHANGELOG.md) for the full story. |
| `convert-v5.1.0D.sh` | 5.1.0D | New feature, both platforms: a kept output whose final measured VMAF lands below a 85.00 floor is now flagged for human review (tag marker + `low_quality_review.txt` log on bash; new `Get-VesFinalVmaf`/`Write-VesLowQualityFlag` infrastructure on the Windows port, which had no post-encode quality-review mechanism at all before this) instead of being silently treated as a normal successful pass — matters most for must-eliminate legacy sources where the existing format-elimination override already keeps a lower-quality output rather than the undesirable container. See [CHANGELOG.md](CHANGELOG.md) for the full story. |
| `convert-v5.1.0E.sh` | 5.1.0E | Team review (Gemini/Codex/Cursor, independent parallel review) of v5.1.0D found the Windows half of that feature was completely inert in production: ffmpeg's filter-option parser splits on `:`, so the Windows temp path passed to `log_path=` broke at the drive letter and every VMAF sample silently failed. Fixed by using a bare relative filename with the ffmpeg process's working directory set to the temp folder, empirically verified end-to-end on PRINCE before fleet redeploy. Also fixed: a missing upscale-target passthrough that would've scored upscaled outputs at mismatched resolutions, duration-probe subprocess hygiene (stderr drain + dispose) to match the project's existing pattern, and a bash numeric-format guard against a theoretical `nan`-coerces-to-zero false positive. See [CHANGELOG.md](CHANGELOG.md) for the full story. |
| `convert-v5.1.0F.sh` | 5.1.0F | Follow-up triage of the remaining review gaps. Most severe: the Windows port's main convert-mode scan (batch and pipeline both) never filtered out its own prior outputs, so every full-library rescan re-encoded "Title.AV1-WIN.mkv" into "Title.AV1-WIN.AV1-WIN.mkv" and beyond, indefinitely — fixed on both platforms, including teaching bash's own `is_derived_output` to recognize the Windows naming convention for shared-library safety. Also fixed a dead null-ambiguity guard in `Test-VesDurationsMatch` (a `[double]` parameter was silently coercing `$null` to `0.0` before the check ever ran, so a doubly-failed duration probe on a stalled NAS could score as "matched" instead of "can't confirm"). A few other reviewed findings were confirmed safe as-is (fail-open-on-ambiguity design, working as intended) or logged as real but out-of-scope design gaps for a future pass (cross-OS lock/done-log non-interop, no Windows size-guard/bake-off, Dolby Vision Profile 5 handling). See [CHANGELOG.md](CHANGELOG.md) for the full story. |
| `convert-v5.1.0G.sh` | 5.1.0G | Remediation of v5.1.0F's four logged gaps, per explicit direction to close the Windows/bash feature-parity gap wherever practical. Cross-platform title-lock claim peek (bash and Windows use different lock primitives for the same purpose; neither recognized the other's before this, so both could claim and encode the same title simultaneously on a shared library). Cross-platform existing-output recognition (closes the remaining *sequential* duplicate-work gap the lock peek alone couldn't). Windows gained a real size-guard + x265 fallback (previously kept any duration/structure-valid AV1 regardless of size) via a `Build-VesFfmpegVideoArgs`-codec-parameterized refactor, empirically verified on PRINCE across all three outcomes. Dolby Vision Profile 5 on Windows: added a missing libplacebo capability probe (every P5 source was routed to human-review even on builds that support the conversion) and fixed the caller never checking `Build-VesFfmpegVideoArgs`'s documented `$null` contract at all. `Write-VesLowQualityFlag` gained bash-style symlink-log hardening. Testing this pass caught three more real bugs along the way: a PowerShell `-replace` operator-precedence bug that silently no-op'd a filename transform, a DoVi-null-misattribution bug (not every `$null` from that function means DoVi P5), and confirmed several "reviewed, working as intended" non-fixes. See [CHANGELOG.md](CHANGELOG.md) for the full story. |
| `convert-v5.1.0H.sh` | 5.1.0H | E2E confidence review (Gemini/Codex/Cursor, independent parallel review) of v5.1.0G's new size-guard/x265 fallback found two critical bugs, both confirmed by multiple reviewers. Data-loss: `-InPlace` + an oversized-or-failed AV1 encode could delete the only remaining copy of a title before the x265 fallback attempt, permanently losing it if x265 also failed — fixed by skipping the size-guard/x265-fallback dance entirely for `-InPlace` targets, matching this port's pre-v5.1.0G behavior exactly. Self-reencode cascade: the new x265 fallback's `Title.X265-WIN.mkv` naming wasn't recognized as a derived output anywhere (same bug class as v5.1.0F's `.AV1-WIN.mkv` one, for the newly-added path) — fixed in all five places that needed it on both platforms. Also fixed: a codec-claim-proof gap for cross-platform outputs before bash's deletion-safety check, a path-normalization bug in the existing-output candidate comparison, an orphaned-invalid-file cleanup gap in the must-eliminate AV1 retry, and Dolby Vision Profile 5 handling being silently dropped on the disc-source path. Both critical fixes empirically re-verified on PRINCE before fleet redeploy. See [CHANGELOG.md](CHANGELOG.md) for the full story. |
| `convert-v5.1.0I.sh` | 5.1.0I | Real bug found resuming a live 27-file fleet test: `-p` targeting a real, existing file whose name contains literal `[`, `]`, `*`, or `?` characters (a common release-group tag like `[tvu.org.ru]`) was silently reinterpreted as "directory + trailing name-glob" instead of the literal file, since `split_path_trailing_glob()`'s glob-metacharacter heuristic can't distinguish an actual unexpanded shell glob from a real filename that just happens to contain those characters. Fixed by checking literal file existence first, matching the same precedence `SINGLE_FILE_MODE` detection already uses a few lines later. Verified both directions (the real file that surfaced this, and a genuine glob-pattern invocation). See [CHANGELOG.md](CHANGELOG.md) for the full story. |
| `convert-v5.1.0J.sh` | 5.1.0J | Windows port: a live fleet failure (PRINCE, mid-batch) traced to a NAS/SMB-specific bug: freshly-created files over SMB on this NAS get a broken default ACL, making a second reopen-and-append unreliable -- the same root cause already fixed once for the done-log, now also fixed for `Write-VesLowQualityFlag`/`Write-VesBadSourceFlag` (one-file-per-entry, `Set-VesEveryoneReadWrite` ACL-widen helper) plus the higher-blast-radius pipeline-mode `pipeline-ready.txt` queue and `VesSubtitleFilter.psm1`'s stripped-subtitles log (hardened ahead of need). Also fixed a pre-existing bug where flag writers received the whole scan root instead of each title's own folder. Bash: the human-review logs (`bad_sources.txt`, `low_quality_review.txt`, `corrupt_files.txt`, `reconvert_files.txt`, `stripped_subtitles.txt`, `multipart_mismatch.txt`) now serialize their appends through the existing shared mutex, closing a latent multi-host NFS concurrent-append race the done-log already avoided. Both platforms' fix sets team-reviewed (Gemini/Codex/Cursor) before and after implementation. See [CHANGELOG.md](CHANGELOG.md) for the full story. |
| `convert-v5.1.0K.sh` | 5.1.0K | RAM disk staging, fleet-wide audit and fixes (no more WSL2 to work around, so this got more aggressive): raised the RAM disk sizing target from 40% to 50% of available memory on both platforms; removed Windows' 32GB hard cap (`VesRamDisk.psm1`), which was silently clamping high-RAM machines like RANDYJ's 144GB well below what 50% would actually allow; fixed a real bug where macOS's available-memory estimate counted only `vm_stat`'s "Pages free" (which macOS deliberately keeps near zero, favoring cache) instead of a Linux-`MemAvailable`-equivalent figure — confirmed in production this starved Crystalight/MARLONJ's RAM disk down to 1.5GB on a 64GB machine; now includes inactive+speculative (reclaimable) pages, giving ~20GB instead. Biggest fix: none of the 3 live Windows fleet machines' actual launch invocations were ever passing `-UseRamDisk` (an opt-in switch) — all three had been running with zero RAM-backed staging the entire time. `-UseRamDisk` is now on by default (matching bash's own default-on `CONVERT_NO_RAMDISK=false`), with a new `-NoRamDisk` opt-out mirroring the existing `-Pipeline`/`-NoPipeline` convention. Verified via a real end-to-end test on PRINCE (RAM disk created, written to, and cleanly torn down) before fleet deploy. See [CHANGELOG.md](CHANGELOG.md) for the full story. |
| `convert-v5.1.0L.sh` | 5.1.0L | Windows port: a real 15-episode PRINCE batch finished "12 ok, 3 failed" with zero log explanation anywhere for the 3 -- root-caused to `Invoke-VesEncodeAndValidate`'s size-guard/x265-fallback logic silently discarding a fully-completed x265 output (real encode work, thrown away with no trace) whenever both AV1 and x265 exceeded their size guards on a non-must-eliminate source. Fixed with explicit logging and routing through the existing `NeedsHumanReview` mechanism at all 3 silent-fallthrough sites in that function. Also fixed: the same PRINCE batch showed every single job (not just the 3 failures) hitting `Set-Content ... Access is denied` on its `ffmpeg-logs` stderr sidecar -- the directory-creation equivalent of the file-level NAS ACL bug fixed in v5.1.0J, now also fixed for `VesTwoStageEncode.psm1` and `VesLegacyFallback.psm1`'s error-log directories. Team review (Gemini/Codex/Cursor) caught a real regression in the first version of the size-guard fix before it shipped -- all three independently found it would have durably blacklisted ordinary transient/retriable dual-encode failures (network blips, NAS hiccups) as permanent skips, not just genuine size-guard policy rejections; fixed by gating the human-review flag on both encodes having actually succeeded, not merely both having failed to produce a keepable result. See [CHANGELOG.md](CHANGELOG.md) for the full story. |
| `convert-v5.1.0M.sh` | 5.1.0M | New feature, both platforms: real field-based deinterlace (QTGMC via VapourSynth) for confidently-detected genuine interlace within the `vintage` profile, replacing the previous no-op (no auto-deinterlace filter existed at all before this). New source-traits detector (`modules/ves-source-traits.sh` / `VesSourceTraits.psm1`) classifies each source's field mode (progressive/telecine/interlaced/ambiguous) via `idet` probes at 3 complexity-representative sample points; only a confident `interlaced` read on the vintage profile triggers QTGMC, with a real `bwdif` ffmpeg-filter fallback (also new -- didn't exist before) when the QTGMC toolchain is unavailable or fails. Fleet-wide QTGMC toolchain deployed and verified on 9 of 10 machines (Fedora/Ubuntu/macOS/Windows); RANDYJ has a confirmed, deferred AVX2 hardware gap and relies on the bwdif fallback. Multi-tool review (Gemini/Codex/Cursor) before this shipped found a critical bug on the bash side -- under `set -euo pipefail`, a bare failing command-substitution assignment aborted the whole encode on any QTGMC failure instead of falling back to bwdif, reproduced and fixed -- plus real fixes for single-quote-in-filename crashes, HDR/DoVi metadata being read from the wrong (QTGMC-stripped) file, VMAF-target profile misclassification, a macOS availability-check false-negative, an unhonored `--no-auto-detelecine` flag, global-metadata loss, and anamorphic SAR/DAR loss risk. Also caught and fixed independently on the bash side during this work: a `ChromaEdi='none'` QTGMC parameter that was silently corrupting every processed frame's chroma into visible green noise -- present since the very first "successful" test this whole effort ran, invisible to every prior check because none of them had ever actually rendered and looked at a frame, only checked ffprobe metadata. Windows port built from scratch alongside bash (no source-traits or QTGMC support existed on Windows before this), verified end-to-end on real hardware (ELVIS) including a Windows-specific bug found and fixed during that verification (unsuppressed `.NET` boolean return values leaking onto a function's pipeline output, corrupting the returned intermediate-file path). See [CHANGELOG.md](CHANGELOG.md) for the full story. |
| `convert-v5.1.0N.sh` | 5.1.0N | Four real bugs found and fixed in v5.1.0M's QTGMC feature, all caught only by running a genuine, unmocked, real-content single-file encode through the actual production script (every prior v5.1.0M test used short manual clips or bypassed real profile detection): (1) bash-only validation-timeout bug meant QTGMC's real transcode had never actually succeeded on real production-length content -- it used a bounded, file-size-scaled timeout wrapper for a call that reads from a pipe, so the scaling logic always fell back to a short base timeout that killed the transcode mid-write; fixed by switching to the same unbounded wrapper the real encode itself uses. (2) Both platforms: the final quality-gate VMAF measurement compared the encoded output against the raw interlaced/telecined original instead of QTGMC's deinterlaced intermediate -- a clean deinterlaced frame and a raw combed frame are structurally different images by design, and this measured VMAF 4.9 for a genuinely correct encode, which would have falsely flagged every future QTGMC title for human review; fixed with a same-representation VMAF computed while the intermediate still exists. (3) Both platforms: every successful QTGMC job permanently leaked its multi-GB lossless intermediate -- only the failure path ever cleaned it up; fixed with proper cleanup on the success path too. (4) Bash-only, introduced by this session's own fix for (3): a `trap ... RETURN` fired twice (once correctly, once again for the caller's own return since the QTGMC call is a tail statement) and crashed the job with "unbound variable" immediately after a fully successful encode; fixed by making the trap self-clearing. All four verified via a real production run: QTGMC succeeded, the AV1 encode completed, tagged VMAF 92.2 (plausible, not a false below-floor flag), and the staging directory was gone after the job finished. See [CHANGELOG.md](CHANGELOG.md) for the full story. |
| `convert-v5.1.0O.sh` | 5.1.0O | Multi-tool review (Gemini + Codex + a third independent reviewer) of v5.1.0N's own fix set, requested as a confidence pass. Found real problems in the fixes themselves: (1) **CRITICAL, Windows only** -- the v5.1.0N final-VMAF and staging-leak fixes never actually worked at all, because `$videoSrc`/`$qtgmcIntermediate` were set in one function but read in a different one that doesn't share PowerShell locals -- every successful Windows encode was silently logged as a failure and re-queued forever, a regression already on the deployed Windows fleet; fixed by moving the VMAF measurement and cleanup inside the function that actually owns those variables. (2) Bash: a hardcoded `target_height=0` broke the same-representation VMAF for any upscaled title (genuine interlaced vintage content is routinely upscaled during the real encode, producing a dimension mismatch libvmaf rejects outright); fixed to use the already-computed upscale target. (3) Bash: an unguarded `rm -rf` inside the new `trap ... RETURN` could silently abort the whole script under `set -e` on a failed delete; fixed with `|| true`. (4) Bash: the VMAF cache could in principle leak a stale value from a discarded encode attempt onto a different attempt's kept output for the same source; fixed by keying on destination path too and clearing at every new attempt's start. (5) Bash: the v5.1.0N timeout fix left the QTGMC transcode pipe completely unbounded and untracked for interrupts (a bash pipe-subshell quirk); fixed with a real, source-duration-scaled timeout, verified to kill the whole process group. All five re-verified together via a second real production run. See [CHANGELOG.md](CHANGELOG.md) for the full story. |
| `convert-v5.1.0P.sh` | 5.1.0P | A further review round found v5.1.0O's own timeout fix didn't actually work either: `run_with_timeout`'s `--foreground` flag (needed there for a different, legitimate reason) does not create a new process group per GNU timeout's own docs, so it never protected against a hung QTGMC transcode leaving orphaned processes -- fixed by resolving the timeout binary directly and invoking without `--foreground`, confirmed via direct process-group-kill test. Also fixed: a `set -e` bare-command risk in the new timeout code; a VMAF-cache entry-clear (added in v5.1.0O) that broke a real must-eliminate-format AV1-stash-and-later-salvage scenario, removed since the existing source+destination match was already sufficient; and a real, live gap found on the fleet's one macOS machine (MARLONJ) which had no `timeout`, `gtimeout`, or `setsid` at all -- fixed by installing `gtimeout` there directly plus hardening the timeout-resolution code to check fixed Homebrew paths (the `worker` service account has no PATH-setting profile files at all) and to probe for `--kill-after`/`setsid` support before relying on them. All four findings verified across four consecutive real production single-file encodes and a final clean multi-tool review round. See [CHANGELOG.md](CHANGELOG.md) for the full story. |
| `convert-v5.1.0Q.sh` | 5.1.0Q | Found monitoring a live 9-machine fleet batch: `remux_copy_to_mkv()` (the shortcut path for sources already in the desired codec, only needing a container change) had no fallback when a subtitle track was non-empty but internally malformed -- a single bad `mov_text` track failed the entire remux (video+audio lost too), reproduced on 5 real "Marvel's Jessica Jones" S03 mp4 sources on Plex (`Task finished with error code: -22`). Fixed with the same retry-without-subtitles fallback the main two-stage encode path (`ffmpeg_encode()`) already had; verified against a real failing file -- fallback produces a full-length, `mkvalidator`-clean MKV. See [CHANGELOG.md](CHANGELOG.md) for the full story. |
| `convert-v5.1.0Y.sh` | 5.1.0Y | Found and root-caused a real VMAF false-positive bug during further fleet validation: ffmpeg's MKV muxer relabels frame timestamps with a small constant offset relative to the source, causing timestamp-seek-based quality comparisons to land on adjacent (not matching) frames — confirmed via direct packet-timestamp inspection and visual/SSIM comparison on 3 real files wrongly flagged "BELOW FLOOR" (44-76 measured vs. genuinely fine). Fixed the provably-safe case: lossless stream-copy remuxes now skip VMAF measurement entirely (`$lossless` flag through `finalize_mkv_output`), since a byte-identical copy cannot have a real quality problem regardless of the muxer bug. The harder case (a real re-encode's final-quality check hitting the same bug) is documented with the root cause and a promising but not-yet-precise lead, left unpatched rather than risk a regression to the shared VMAF comparator every real encode depends on. Bash only — no Windows code path exists yet for the fixed case. See [CHANGELOG.md](CHANGELOG.md) for the full story. |
| `convert-v5.1.1A.sh` | 5.1.1A | Diagnostic safeguard for the frame-duplication defect found auditing a 38-file fleet-wide below-VMAF-floor backlog (all 38 re-verified genuine, zero false positives; one file deep-investigated found real duplicate packets baked into the AV1 output — invisible to a still-frame comparison but catastrophic to VMAF). Root cause not conclusively pinned down at the time — see v5.1.1B, which found and fixed it. New `detect_output_frame_duplication()`/`Get-VesOutputFrameDuplication` (reusing the existing DTS-delta-variance technique against the output instead of the source) turns a generic "NEEDS REVIEW" flag into a self-diagnosing one. Ported to Windows in full, including the previously-never-ported `_dts_delta_cv` primitive. Also requeued all 38 confirmed-defective files for re-encoding fleet-wide and fixed an unrelated real deployment bug found along the way (a prior RAM-disk fix's config file silently never landed on 5 of 6 Linux/macOS machines). See [CHANGELOG.md](CHANGELOG.md) for the full story. |
| `convert-v5.1.1B.sh` | 5.1.1B | Root cause found and fixed for the v5.1.1A frame-duplication defect. The v5.1.1A diagnostic tag proved the defect reproduces 100% deterministically on fresh re-encode, not from an isolated NAS-contention window: every Stargate Universe S01/Marvel's Runaways S02 episode reprocessed during the requeue came back tagged, across 6 fleet machines spanning both platforms. Isolated via direct clip-level reproduction against a real source file: broken from t=0, clean from t=30s (same duration); ruled out scene-change detection and encoder preset as factors (identical duplicate count either way, ruling out SVT-AV1-internal state). The fix: `-fps_mode cfr` instead of `-fps_mode passthrough` on the stage-1 encode — `passthrough` was blindly propagating a decoded-PTS irregularity in these sources' opening ~30 seconds (invisible at the container/packet level, confirmed via a whole-file packet scan of the source) into a persistent duplicate-frame pattern for the rest of the job. `passthrough` had been added 2026-07-31 as an explicitly unconfirmed mitigation for an unrelated bug whose real fix was the two-stage restructure shipped days later — it was never actually load-bearing. Changed on both platforms, stage-1 only (stage-2 remux is an unaffected `-c:v copy` stream copy). See [CHANGELOG.md](CHANGELOG.md) for the full story. |
| `convert-v5.1.1C.sh` | 5.1.1C | Fixed the real-encode VMAF muxer-timestamp measurement bug (task #172), found at scale via a fleet-wide re-scan that turned up an entire 52-episode series (Falling Skies) scoring well below the VMAF floor despite healthy CRF-search predictions. Root cause: ffmpeg's MKV muxer relabels the output's PTS with an offset vs the source, so two independent same-nominal-timestamp seeks into source/output can land on different real-content frames. Two earlier fix attempts (unconditional measured-offset compensation, then max-of-2-candidates) were each disproven by real tests — including a genuine regression the naive fix caused on a real file — before landing on the shipped design: search every whole-frame offset in a small range (`VMAF_ALIGN_SEARCH_FRAMES`/`-AlignSearchFrames`, default 3) per sample window and keep whichever scores highest, extending the codebase's existing "misalignment can only ever drag a score down, never inflate it" safety pattern from a 2-candidate check to a real local search. Verified against 4 real cases with no regressions. Costs ~5-8 minutes per file (accepted deliberately, per explicit user direction — a once-per-file diagnostic tag, not a hot path). See [CHANGELOG.md](CHANGELOG.md) for the full story. |
| `convert-v5.1.1D.sh` | 5.1.1D | Fixed a validation-retry bug found during a 20-file movie-length fleet stress test: `_run_timeout_retry`/`Invoke-VesWithTimeoutRetry` retried a timed-out `mkvalidator`/`ffprobe`/`mkvmerge` call using the same timeout budget every attempt, so a large movie source that genuinely needed more time than one size-scaled budget allowed got permanently skipped as "possibly stalled" even though nothing was actually stalled — all 3 attempts (1 initial + 2 retries) failed identically. Fixed by doubling the timeout on each retry instead of reusing it, on both platforms. Also found (separate, not fixed this release): Windows encodes never get an embedded quality tag on passing files because no MKVToolNix is provisioned on the Windows fleet — the safety-critical below-floor flagging is unaffected. See [CHANGELOG.md](CHANGELOG.md) for the full story. |
| `convert-v5.1.1E.sh` | 5.1.1E | Routed live ffmpeg diagnostic stderr off the NAS onto local/RAM-disk staging (both platforms), found while investigating a user report that RAM-disk utilization looked low during a 9-machine weighted stress test. Empirically confirmed the per-title stderr log grows continuously throughout an encode (~every 0.5s, for however long a real encode runs) and was resolving to the NAS folder next to the source by default — real, avoidable NFS write traffic for a diagnostic stream nobody reads while the job is healthy. Fixed by routing the live tee to the same local/ramdisk directory the encode's own binary output already stages to, copying the finished log back to the NAS sidecar only if non-empty. Scope per explicit user direction: only multi-connection-blocking (title-locks), progress-tracking (resume-state/done-log, the Windows `-progress` file), and human-inspection (`bad_sources.txt`, `corrupt_files.txt`, etc.) files stay on the NAS — everything else "live" during the encode now stays local. See [CHANGELOG.md](CHANGELOG.md) for the full story. |
| `convert-v5.1.1F.sh` | 5.1.1F | Second pass of the same NAS-load/RAM-disk audit (full sweep of every live-encode write path, not just the one already fixed in v5.1.1E), per explicit user direction to triage "even small ones" before restarting the stress test. Real bug found: `qtgmc_deinterlace_to_intermediate()`'s multi-GB lossless intermediate used `CONVERT_RAMDISK_DIR` (a rarely-set user override, empty by default) instead of `RAMDISK_JOB_DIR` (the actual runtime-resolved per-job ramdisk) — fell through to plain `/tmp` on every real run, harmless on Linux (tmpfs) but real local disk on macOS, sharing neither the job's ramdisk disk-budget nor its teardown lifecycle. Fixed to use `RAMDISK_JOB_DIR` directly. Also, for consistency: every CRF-search sample-encode temp file in `modules/ves-vmaf-crf-search.sh` now prefers `RAMDISK_JOB_DIR` too (already RAM-backed via `/tmp` on Linux; closes the same macOS gap). Everything else audited (VMAF comparison, the title-lock heartbeat, HandBrake temp clips, all Windows `.psm1` temp-file usage) was already correct. See [CHANGELOG.md](CHANGELOG.md) for the full story. |
| `convert-v5.1.1G.sh` | 5.1.1G | Closed a real Windows/bash feature-parity gap, user-flagged as high severity: the proactive per-source VFR/CFR detection + baseline self-VMAF check (bash's v5.1.0W) had no Windows port at all. Not a quality-safety hole in the VMAF comparison math itself (already correctly ported in `Get-VesFinalVmaf`) — the missing piece was the proactive detection/logging/ambiguous-flagging layer that runs before CRF search. Fixed by porting `Get-VesSourceFrameRateMode`, `Get-VesSourceBaselineVmaf`, and `Write-VesSourceTraitsAmbiguousFlag`, reusing already-ported helpers rather than duplicating logic, wired into `convert.ps1` at the same point bash uses the equivalent pair. Verified live on RANDYJ against real data: numerically identical to bash's result on the same file. No bash changes this release — version bumped anyway for the shared bash/Windows version number. See [CHANGELOG.md](CHANGELOG.md) for the full story. |
| `convert-v5.1.1H.sh` | 5.1.1H | Two real bugs found root-causing a genuine PRINCE production failure during the full-history regression test ("A Clockwork Orange (1971)" failed after ~13 hours): RAM-disk space exhaustion silently corrupted an in-progress x265 stage-1 encode (caught correctly by existing post-encode validation, no bad output kept, but real compute wasted), and separately, the AV1 attempt's own crash diagnostics were unrecoverable because both AV1 and x265 fallback attempts reused an identical PID-only log filename. Fixed both, both platforms: diagnostic stderr logs now write to genuine local disk, never the RAM disk (reserved exclusively for encode data); codec is now part of every diagnostic log filename so a fallback retry can't clobber the failed attempt's evidence. See [CHANGELOG.md](CHANGELOG.md) for the full story. |
| `convert-v5.1.1I.sh` | 5.1.1I | Explicit user hypothesis, confirmed and fixed: an AV1-to-x265 fallback reuses the same RAM-disk staging directory across both codec attempts (only torn down at job end), so a crashed AV1 encode whose own cleanup didn't run could leave partial data sitting there competing with the following x265 attempt for the same limited space — directly contributing to v5.1.1H's root-caused failure. Fixed on both platforms: before any x265 fallback begins, the RAM disk staging area is now explicitly swept and anything found is moved to a local-disk holding directory. Also found during this session's audit: ELVIS's "Mad Heidi" run was previously misreported as a clean success — it actually failed with the identical RAM-disk `ENOSPC` signature, traced to a stale unrelated log entry being read by mistake; needs a re-run. See [CHANGELOG.md](CHANGELOG.md) for the full story. |
| `convert-v5.1.1J.sh` | 5.1.1J | **Current** — Root-caused why the RAM-disk-exhaustion failures kept happening: a real PRINCE crash (`SvtMalloc[fatal]: allocate memory failed` on a true 4K/10-bit source) turned out distinct from the earlier `ENOSPC` failures — the encoder's own internal memory (SVT-AV1's picture-buffer pool, ~7.6GB at that resolution/GOP alone) getting squeezed because the RAM disk's flat 60%-of-available formula claimed the majority of free memory before the encoder even started. Fixed on both platforms with two-tier sizing: below 64GB available, a flat 12GB cap regardless of how much more is technically free; at/above 64GB, 45% of available (down from 60%). All thresholds are named, overridable config constants. Verified against 5 realistic scenarios confirming correct tier selection and safety clamps. Syntax-verified only, not yet exercised live. See [CHANGELOG.md](CHANGELOG.md) for the full story. |
| `convert-v5.1.0Z.sh` | 5.1.0Z | Raised default RAM-disk sizing from 50% to 60% of available memory (`CONVERT_RAMDISK_PCT` / `PercentOfAvailable`, both platforms) after a real `ENOSPC` failure on PRINCE: a 4K movie's AV1 encode completed successfully after ~4h20m but died at the final remux stage with "No space left on device" (confirmed twice — with and without subtitles), losing all that compute before falling back to x265. Root cause: the two-stage pipeline's stage-1 intermediate and stage-2 final output briefly coexist on the same RAM disk, so peak usage approaches ~2x the final file size — easily exceeding a 50%-of-available RAM disk on a large title. Single-line default change on each platform, no caller overrides it, still leaves 40% of available memory for the encoder's own working set. See [CHANGELOG.md](CHANGELOG.md) for the full story. |
| `convert-v5.1.0X.sh` | 5.1.0X | Fleet-validation triage pass, both platforms: (1) fixed a real hang in Windows' orphan reaper — `Get-ChildItem -Recurse -Filter` silently ignores `-Filter` when the target is a single file, so single-file `-SearchPath` invocations were feeding the multi-GB source video itself to `Get-Content` as a "flag candidate," hanging for 20+ minutes reading it as text (invisible until today's SMB ACL fix let the read actually proceed instead of failing fast); (2) root-caused the fleet's shared "Permission denied"/`icacls` ACL-widen failures on freshly-created `ffmpeg-logs/` sidecar dirs to Samba's default `create mask`/`directory mask` on all 3 Media shares — fixed server-side via TrueNAS `auxsmbconf` (`create/directory mask = 0777`, `force create/directory mode = 0777`), verified live; (3) removed `enable-hdr=1` from both platforms' svtav1-params — never a real SVT-AV1 option on any version checked, silently logged-and-ignored on every HDR encode this project has ever run, first noticed when SVT-AV1 4.2.0 (fleet standard is v4.1.0) logged the parse failure loudly; (4) fixed a Windows-only local-disk staging-directory leak (ELVIS alone had 55 orphaned dirs) — the local staging fallback never recorded an owner, so a crashed job's scratch dir could never be reclaimed; added an ownership marker + orphan-reaper sweep matching the RAM-disk staging path's existing pattern. See [CHANGELOG.md](CHANGELOG.md) for the full story. |
| `convert-v5.1.0W.sh` | 5.1.0W | New proactive per-source VFR/CFR detection (`detect_frame_rate_mode`, packet DTS-delta variance) + baseline self-VMAF (`measure_source_baseline_vmaf`) before CRF search, unconditional across every profile, added after a fleet-wide re-verification pass found and auto-corrected ~130 files mistagged "NEEDS REVIEW" by the pre-v5.1.0S/T VMAF measurement bug. Also this session: fixed the NAS root-squash bug causing "Permission denied" writing shared sidecar dirs (TrueNAS API, `maproot_user=root`); fully root-caused and fixed the Windows SMB `icacls` failures — TrueNAS's own `ixnas` VFS module was rejecting every ACL-set syscall (`Operation not permitted`) regardless of Windows-side auth, fixed via a native `acltype=posix` dataset switch on all three Media shares (survives updates/reboots), verified with zero `icacls` failures under real production traffic on all 3 Windows machines. Bash only — PowerShell port is a follow-up. See [CHANGELOG.md](CHANGELOG.md) for the full story. |
| `convert-v5.1.0V.sh` | 5.1.0V | Generalized v5.1.0U's fix: new `match_source_permissions()` makes every written file/dir match its source path's real mode/owner/group instead of a hard-coded guess, wired into `finalize_mkv_output` (every output, fleet-wide) and `ensure_shared_sidecar_dir`. chmod half works reliably; chown is confirmed hard-blocked server-side by this NAS for every client (root included) — not client-fixable, kept best-effort. See [CHANGELOG.md](CHANGELOG.md) for the full story. |
| `convert-v5.1.0U.sh` | 5.1.0U | Fixed real "Permission denied" errors on bash writing shared NAS sidecar dirs (`ffmpeg-logs/` etc.): a freshly-created dir doesn't inherit this library's 777 convention and ends up 0775/nobody:8080 on this NAS, locking out every other writer. New `ensure_shared_sidecar_dir()` self-heals via chmod, falling back to passwordless sudo when needed (bash never had the Windows port's equivalent fix). See [CHANGELOG.md](CHANGELOG.md) for the full story. |
| `convert-v5.1.0T.sh` | 5.1.0T | Corrected v5.1.0S's VMAF frame-rate fix (forcing the fps filter unconditionally regressed some Battlestar Galactica episodes; now measures each window both ways and keeps the higher score) and fixed a severe uncaught-exception bug that destroyed 100% of a 36-episode RANDYJ batch (a diagnostic stderr-log write failure was aborting already-completed encodes; now non-fatal). See [CHANGELOG.md](CHANGELOG.md) for the full story. |
| `convert-v5.1.0S.sh` | 5.1.0S | Fixed a false-positive "below VMAF floor" flag on VFR sources: `measure_final_vmaf()`/`Get-VesFinalVmaf` compared misaligned frames when the source had duplicate/dropped frames drifting its timing from the CFR output, silently scoring near 0 and dragging the pooled mean down (dozens of episodes across 5 shows/5 machines wrongly flagged — the actual encodes were fine). Fixed by normalizing both streams to the source's frame rate before comparison; verified 53.0 → 92.2 on the same real file. See [CHANGELOG.md](CHANGELOG.md) for the full story. |
| `convert-v5.1.0R.sh` | 5.1.0R | Two fixes found monitoring live batches on PRINCE and JJACKSON. (1) The subtitle text-decode probe now short-circuits on confirmed real content on both platforms (was always running to full completion; PRINCE was repeatedly burning its full 3x60s retry budget across many Sabrina episodes) -- verified real speedup on a real file, 2+ minutes → 43.5s. (2) `season_retry_pass()` now self-verifies it processed every promised retry candidate, after JJACKSON silently completed only 1 of 3 promised season-retry episodes with zero trace in the log -- root cause not pinned down despite an isolated repro of the loop behaving correctly, so this makes any recurrence loud instead of silent. See [CHANGELOG.md](CHANGELOG.md) for the full story. |
