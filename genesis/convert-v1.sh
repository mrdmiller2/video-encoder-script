...
readarray -t raw < <(find . -type f -name "*.avi" -o -name "*.mp4" -o -name "*.mkv" -o -name "*.ts")
...
do
  ...
  type=$(ffprobe -v error -select_streams v:0 -show_entries stream=codec_name -of default=noprint_wrappers=1:nokey=1 "$r")
    if [ "$type" != "av1" ]; then
        ...
        HandBrakeCLI -Z "AV1 MKV 2160p60 4K" --all-subtitles --all-audio -e svt_av1_10bit -q 26 --encoder-preset 4 -f mkv -m -O -E opus --lapsharp=light --ab 192 --enable-hw-decoding nvdec --encopts="enable-qm=1:qm-min=0:keyint=15s:scd=1:aq-mode=2:mbr=10m:enable-variance-boost=1:enable-overlays=1" --subtitle-burned=none --subtitle-default=none -6 opus -i "$r" -o "$r-av1"
        
        echo "Resting for 5 seconds"
        sleep 5
        echo "Editing the Metadata"
        mkvpropedit "$r-av1" -v -e info --set "title=$crispy"
