...
do
  ...
  type=$(ffprobe -v error -select_streams v:0 -show_entries stream=codec_name -of default=noprint_wrappers=1:nokey=1 "$r")
    if [ "$type" != "av1" ]; then
        ...
        flatpak run --command=HandBrakeCLI fr.handbrake.ghb -Z "AV1 MKV 2160p60 4K" --all-subtitles --all-audio -e svt_av1_10bit --encoder-preset 5 -q 35 -f mkv -m -O -E opus --lapsharp=light --ab 100 --enable-hw-decoding nvdec --encopts="enable-qm=1:qm-min=0:scd=1:enable-tf=0:keyint=15s:aq-mode=2:enable-variance-boost=1:enable-overlays=1" --subtitle-burned=none --subtitle-default=none -6 opus -i "$r" -o "$r-av1"
