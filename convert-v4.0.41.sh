#!/usr/bin/env bash
# convert-v4.0.41.sh — Organize movie folders, transcode TV/movies to AV1/x265 MKV.
# Version: 4.0.41
...
Linux / WSL — pick the block that matches your distro:
  ...
  Debian / Ubuntu / WSL (apt):
    ...
    # mkvalidator is not in apt — use Homebrew on Linux (bottles) or build once:
...
EOF
  ...
  Suggested for this host:
    ...
    brew install mkvalidator   # after installing Homebrew on Linux, for Matroska structure checks
EOF
  ...
  Suggested for this host:
    sudo dnf install -y ffmpeg mkvtoolnix HandBrake-cli python3 grep ripgrep
    brew install mkvalidator   # optional; enables full Matroska structure validation
EOF
  ...
  Suggested for this host:
    sudo pacman -S --needed ffmpeg mkvtoolnix handbrake python grep ripgrep
    yay -S mkvalidator   # or: brew install mkvalidator
EOF
      fi
      ;;
    windows)
      cat >&2 <<'EOF'
Windows / Cygwin:
  winget install Gyan.FFmpeg HandBrake.HandBrake MKVToolNix.MKVToolNix Python.Python.3.12
  # or Chocolatey:
  choco install ffmpeg handbrake mkvtoolnix python grep
  # mkvalidator: brew install mkvalidator (if using Homebrew/Linux env) or Matroska win64 zip
EOF
      ;;
    *)
      cat >&2 <<'EOF'
...
report_tool_checklist() {
  ...
  if [ "$HAS_MKVALIDATOR" = true ]; then
    ...
    _tool_row "mkvalidator" false false "brew install mkvalidator (macOS/Linux bottles); Arch AUR: mkvalidator"
  fi
  if command -v python3 >/dev/null 2>&1; then
    HAS_PYTHON3=true
...
discover_tools() {
  ...
  if [ "$HAS_MKVALIDATOR" = true ]; then
    ...
    warn "mkvalidator not found — EBML bounds still run; install via: brew install mkvalidator"
  fi
