  ...
  if [ "$anime" = true ]; then
    case "$encoder" in
      svt_av1_10bit)
        ...
        EP_ENCOPTS='enable-qm=1:film-grain-denoise=1:film-grain=6:qm-min=0:scd=1:enable-tf=0:keyint=15s:aq-mode=2:enable-variance-boost=1:variance-boost-strength=3:variance-octile=4:enable-overlays=1:tune=0'
  ...
  case "$codec" in
    av1)
      ...
      if [ "$anime" = true ]; then
        ...
        svtp="$svtp:film-grain-denoise=1:film-grain=6:enable-tf=0:enable-variance-boost=1:variance-boost-strength=3:variance-octile=4:enable-overlays=1:tune=0"
      fi
      if [ "$hdr" = true ]; then
