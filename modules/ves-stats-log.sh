...
# (high/median ratio) that a real 17-title validation against known
# reference titles found had no coherent genre/pacing correlation (see
# docs/DESIGN-6x-chunk-redesign.md on the 6.x-chunk-redesign branch) --
# reverted as a gate, but 17 titles isn't a statistically meaningful
# sample for something this potentially subtle, so rather than abandon
# the signal, this keeps recording the RAW low/median/high values (not
# just one collapsed ratio, so future analysis isn't locked into today's
# statistic choice) for every file processed on BOTH this line and the
# 6.x branch, accumulating from real day-to-day fleet volume instead of
# only deliberate test runs. Same ${JOB_SIDECAR_DIR:-.} location
# convention as sample-prediction-log.tsv/bad_sources.txt. Best-effort:
# a probe failure (e.g. an unusual source ffprobe can't read packet sizes
# for) just skips this file's row rather than blocking anything.
log_source_content_variance() {
  ...
  # showed no genre correlation (see docs/DESIGN-6x-chunk-redesign.md).
