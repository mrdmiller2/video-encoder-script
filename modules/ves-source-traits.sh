...
# log_source_content_variance() (ves-stats-log.sh) for future
# pattern-mining across a much larger real sample than any one
# investigation session can gather by hand. Originated as
# chunk_content_variance_ratio() on the 6.x-chunk-redesign branch (this
# repo's chunk-based redesign line) while investigating whether
# chunk-parallel eligibility should gate on more than duration -- a real
# 17-title validation against known reference titles (see
...
# was reverted there. Per explicit user direction: 17 titles isn't a
# statistically meaningful sample for something this potentially subtle,
# so rather than abandon the signal, this generalized version (lives here
# rather than a chunk-only module, since it isn't chunk-specific, so it
# can run on BOTH this line and 6.x, accumulating from real day-to-day
# fleet volume instead of only deliberate test runs) keeps recording the
# RAW low/median/high values (not just one ratio) for every file that
# finishes processing, so future analysis isn't locked into whichever
# single statistic seemed reasonable today. Reuses the exact same cheap
# probe mechanism find_complexity_sample_points() already relies on
# (ves-vmaf-crf-search.sh) -- ffprobe -read_intervals reading only
# compressed PACKET SIZES across sparse sample windows, no decode and no
# encode, sub-second even on a multi-GB file.
