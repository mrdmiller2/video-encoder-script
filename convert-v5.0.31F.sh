#!/usr/bin/env bash
# convert-v5.0.31F.sh — Organize movie folders, transcode TV/movies to AV1/x265 MKV.
# Version: 5.0.31F
#
...
#   1280x720 or 1920x1080 cap; near-720p sources are left native. Size growth is expected up to
...
try_av1_convert() {
  ...
  if [ "$policy" = keep ]; then
    ...
    if [ "$upscaled" = true ] && [ "$new_sz" -gt "$orig_sz" ]; then
      log "Kept AV1 ($(awk -v o="$orig_sz" -v n="$new_sz" 'BEGIN { printf "%.1f%% of original", (n/o)*100 }'); ${UPSCALE_TARGET_HEIGHT}p upscale, ≤${UPSCALE_MAX_OVERSHOOT_PCT}% growth OK): $out"
    else
  ...
  if [ "$upscaled" = true ]; then
    warn "AV1 output >${UPSCALE_MAX_OVERSHOOT_PCT}% larger than original after ${UPSCALE_TARGET_HEIGHT}p upscale ($(awk -v o="$orig_sz" -v n="$new_sz" 'BEGIN { printf "%.1f%%", (n/o)*100 }')) — trying x265 fallback"
  else
...
try_x265_convert() {
  ...
  if [ "$policy" = keep ]; then
    finalize_mkv_output "$out" "$src" "$title"
    if [ "$upscaled" = true ] && [ "$new_sz" -gt "$orig_sz" ]; then
      log "Kept x265 ($(awk -v o="$orig_sz" -v n="$new_sz" 'BEGIN { printf "%.1f%% of original", (n/o)*100 }'); ${UPSCALE_TARGET_HEIGHT}p upscale, ≤${UPSCALE_MAX_OVERSHOOT_PCT}% growth OK): $out"
    else
  ...
  if [ "$upscaled" = true ]; then
    warn "x265 output >${UPSCALE_MAX_OVERSHOOT_PCT}% larger after ${UPSCALE_TARGET_HEIGHT}p upscale — rejected (keeping original)"
