#!/usr/bin/env bash
# convert-v4.0.9.sh — Organize movie folders, transcode TV/movies to AV1/x265 MKV.
# Version: 4.0.9
...
convert_library() {
  ...
  for f in "${queue[@]}"; do
    ...
    if is_disk_source "$f"; then
      process_disk "$f" || warn "Job failed — continuing queue: $f"
    else
      process_video "$f" || warn "Job failed — continuing queue: $f"
    fi
    end_convert_job "$f" "$CONVERT_JOB_INDEX" "$CONVERT_JOB_TOTAL"
  done
}
