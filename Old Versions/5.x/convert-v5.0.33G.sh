...
must_eliminate_fallback_or_fail() {
  ...
  if is_must_eliminate_format "$src"; then
    ...
    if [ "$(canonical_path "$src" 2>/dev/null || printf '%s' "$src")" = "$(canonical_path "$remux_out" 2>/dev/null || printf '%s' "$remux_out")" ]; then
      flag_bad_source_for_human "$src" "must-eliminate remux fallback path collides with the source itself — needs manual rename/review"
      return 1
    elif [ -L "$remux_out" ]; then
      flag_bad_source_for_human "$src" "must-eliminate remux fallback path is an unexpected symlink — needs manual review"
      return 1
    elif [ -e "$remux_out" ]; then
      flag_bad_source_for_human "$src" "must-eliminate remux fallback path already exists and doesn't look like our own output — needs manual review before overwriting"
      return 1
    fi
    ...
    flag_bad_source_for_human "$src" "both AV1 and x265 transcodes failed, and the plain stream-copy remux fallback also failed — cannot eliminate undesirable format automatically"
  fi
  return 1
}
