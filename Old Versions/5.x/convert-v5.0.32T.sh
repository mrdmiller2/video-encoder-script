#!/usr/bin/env bash
# convert-v5.0.32T.sh — Organize movie folders, transcode TV/movies to AV1/x265 MKV.
# Version: 5.0.32T (see CHANGELOG.md for the full per-version history from
...
X265_PARAMS_CANIME='log-level=error:tune=animation:keyint=240:min-keyint=24:bframes=8:ref=4:rc-lookahead=30:aq-mode=2:psy-rd=2.0:psy-rdoq=1.0'
...
run_ffmpeg_validation() { run_with_timeout "${VALIDATION_TIMEOUT_SECS}" "${FFMPEG_CMD[@]}" "$@"; }
...
# encode job -- this is held only for the few milliseconds a single append
# or rewrite takes, so a bounded spin-wait (not a stale-reclaim scheme) is
# enough: nothing should ever legitimately hold this more than briefly.
...
_shared_mutex_acquire() {
  ...
  while ! mkdir -- "$lockdir" 2>/dev/null; do
    ...
    if [ "$waited" -gt 100 ]; then
      # ~10s of spinning means the lock is stuck (its owner crashed mid-
      # critical-section) -- reclaim rather than block a fleet run
      # indefinitely on a dead lock from a machine that's no longer running.
...
done_log_append() {  # status src
  ...
  if [ -n "$DONE_LOG_FD" ]; then
    _shared_mutex_acquire "${RESUME_DONE_LOG}.appendlock"
    printf '%s\t%s\t%s\t%s\t%s\n' "$st" "${key%%|*}" "${key##*|}" "$src" "$fp" >&"$DONE_LOG_FD" 2>/dev/null || true
    _shared_mutex_release "${RESUME_DONE_LOG}.appendlock"
  fi
  DONE_SET["$src"]="$key#$fp"
}
...
mkv_structure_cache_invalidate() {
  ...
  # review, 2026-07-24).
  _shared_mutex_acquire "${cache}.lock"
  tmpf="$(mktemp "${cache}.XXXXXX")" || { _shared_mutex_release "${cache}.lock"; return 0; }
  ...
  _shared_mutex_release "${cache}.lock"
}
...
mkv_structure_cache_store() {
  ...
  _shared_mutex_acquire "${cache}.lock"
  tmpf="$(mktemp "${cache}.XXXXXX" 2>/dev/null)" || { _shared_mutex_release "${cache}.lock"; return 0; }
  if [ -f "$cache" ]; then
    awk -F '\t' -v p="$dst" '$2!=p { print }' "$cache" >"$tmpf" 2>/dev/null || true
  fi
  printf '%s\t%s\n' "$key" "$dst" >>"$tmpf"
...
validate_mkv_decode_windows() {
  ...
  rc=0
  run_ffmpeg_validation -v error -t "$window" -i "$dst" -map 0:v:0 -f null - 2>"$errf" || rc=$?
  if [ "$rc" -eq 124 ]; then
  ...
  if awk -v d="$dur" -v w="$window" 'BEGIN { exit !(d>w) }'; then
    : > "$errf"
    rc=0
    run_ffmpeg_validation -v error -sseof -"${window}" -i "$dst" -map 0:v:0 -f null - 2>"$errf" || rc=$?
    if [ "$rc" -eq 124 ]; then
...
validate_mkv_subtitle_tracks() {
  local dst="$1"
  local n_subs dur subs_list src_rc forced window start pts rc
  ...
  if [ -z "$subs_list" ] && [ "$src_rc" -eq 124 ]; then
  ...
  # Only track s:0 (the primary/default track) -- same reasoning as audio's
  # track-0-only rule: a secondary commentary/alternate-language subtitle
  # track ending early for legitimate reasons must not fail this. A forced
  # track (foreign-dialogue-only, signs-only) is EXPECTED to stop long
  # before the end -- that's by design, not corruption -- so ask ffprobe's
  # disposition flag directly rather than guessing from cue density.
  local forced_rc
  # Same `&& forced_rc=0 || forced_rc=$?` reasoning as above -- a bare
  # assignment here would crash the script under `set -e` on any ffprobe
  # failure/timeout, never reaching the rc check below.
  forced="$(run_ffprobe -v error -select_streams s:0 -show_entries stream_disposition=forced \
    -of default=noprint_wrappers=1:nokey=1 "$dst" 2>/dev/null)" && forced_rc=0 || forced_rc=$?
  # Team review (2026-07-22, round 2): this probe must not be allowed to
  # fall through unchecked on a timeout or ambiguous error -- a flaky-then-
  # recovering NFS mount could time out/error HERE (on a genuinely forced
  # track) yet still let the tail-gap read below succeed cleanly and find no
  # cues, which would wrongly re-introduce the exact forced-track false
  # positive this disposition check exists to prevent. Treat the same
  # ambiguous-rc set as the tail-gap read below: 124 is a confirmed timeout;
  # anything else non-zero is an ambiguous probe error, not proof the track
  # isn't forced -- both soft-fail (skip this run, don't flag) rather than
  # silently proceeding as if disposition were successfully confirmed clear.
  ...
  start="$(awk -v d="$dur" -v w="$window" 'BEGIN { r = d - w; if (r < 0) r = 0; print r }')"
  # `&& rc=0 || rc=$?`, not `rc=${PIPESTATUS[0]}` after a bare assignment --
  # verified via direct bash testing (2026-07-22) that the bare form crashes
  # the whole script under `set -e` the instant the pipe's pipefail-computed
  # exit status is non-zero, before this rc= line (or anything below it)
  # ever runs. See audio_track_reaches_near_eof for the full explanation.
  pts="$(run_ffprobe -v error -read_intervals "${start}%+${window}" -select_streams s:0 \
    -show_entries packet=pts_time -of csv=p=0 "$dst" 2>/dev/null | head -1)" && rc=0 || rc=$?
  # Unlike audio's near-EOF check, treat rc==124 as a timeout unconditionally
  # here (not only when $pts is also empty) -- this is the exact ambiguity
  # team review found in the original full-scan version (a kill-by-timeout
  # after some cues were already printed must never be read as a confirmed
  # truncation), and there's no equivalent SIGPIPE-from-head-closing-early
  # concern to protect against here since a single-cue near-EOF window
  # essentially never has more than one match queued up when `head -1` closes
  # the pipe.
  if [ "$rc" -eq 124 ]; then
    SUBTITLE_CHECK_TIMED_OUT=true
    return 124
  fi
  # Mirror audio_track_reaches_near_eof's P1 fix (team review, 2026-07-22):
  # an ambiguous non-zero, non-124, non-141 probe error is not itself proof
  # the subtitle track is actually missing cues here -- soft-fail the same
  # as a timeout rather than confirming subtitle_truncated, since the
  # source-side caller can turn a confirmed verdict into a permanent
  # Deferred/ file move.
  if [ -z "$pts" ] && [ "$rc" -ne 0 ] && [ "$rc" -ne 141 ]; then
    SUBTITLE_CHECK_TIMED_OUT=true
    return 124
  ...
  if [ -z "$pts" ]; then
    warn "Validation failed: primary subtitle track s:0 has no cues in the last ${SUBTITLE_SYNC_TAIL_GAP_PCT}% (${window}s) of $dst -- likely a truncated/mismatched subtitle track"
...
av1_source_reencode_sample_decision() {
  ...
  for start in "${uniq_starts[@]}"; do
    ...
    if ! run_ffmpeg -y -nostdin -ss "$start" -t "$sample_len" -i "$sample_src" -map 0:v:0 -map "0:a?" -map "0:s?" -c copy "$clip" >/dev/null 2>&1; then
...
ffmpeg_encode() {
  ...
  if [ "$rc" -ne 0 ]; then
    ...
    args+=(-c:a "$acodec" -b:a "$abr" -af "$(ffmpeg_audio_filter_chain "$acodec")")
    if [ "$acodec" = libopus ]; then args+=(-mapping_family 1); fi
    args+=(-max_muxing_queue_size 8192 -f matroska "$dst")
...
_orphan_write_stage_host_marker() {
  local dir="$1"
  { hostname 2>/dev/null || printf 'unknown\n'; } > "${dir}/.convert-stage-host" 2>/dev/null || true
}
...
must_eliminate_fallback_or_fail() {
  ...
  if [ "$owns_candidate" = true ] && [ -f "$MUST_ELIMINATE_AV1_CANDIDATE" ] && is_must_eliminate_format "$src"; then
  ...
  # forever with no path forward -- defer it for human review instead. This
  # is deliberately scoped to must-eliminate sources only: an ordinary file
  # failing both encoders just stays in place for retry, same as always.
  if is_must_eliminate_format "$src"; then
    flag_bad_source_for_human "$src" "both AV1 and x265 transcodes failed — cannot eliminate undesirable format automatically"
  fi
  return 1
}
...
try_x265_convert() {
  ...
  if [ "$ENCODE_ENGINE" = ffmpeg ] && [ -z "$hb_title" ] && ! is_disk_source "$src"; then
    ...
    encode_dispatch "$src" "$out" "x265" "" "" || { remove_output_only "$out"; must_eliminate_fallback_or_fail "$src" "$title" && return 0; return 1; }
    ...
    handbrake_encode "$src" "$out" "$encoder" "$gpu" "$hb_title" || {
      if [ "$NVIDIA_GPU_COUNT" -gt 1 ]; then
        ...
        handbrake_encode "$src" "$out" "$encoder" "$GPU_HEVC_FALLBACK" "$hb_title" || { remove_output_only "$out"; must_eliminate_fallback_or_fail "$src" "$title" && return 0; return 1; }
      else
        remove_output_only "$out"
        must_eliminate_fallback_or_fail "$src" "$title" && return 0
        return 1
      fi
    }
    ...
    if ! handbrake_encode "$src" "$out" "$encoder" "" "$hb_title"; then
      ...
      encoder="x265"
      handbrake_encode "$src" "$out" "$encoder" "" "$hb_title" || { remove_output_only "$out"; must_eliminate_fallback_or_fail "$src" "$title" && return 0; return 1; }
    fi
  ...
  if [ -n "$MUST_ELIMINATE_AV1_CANDIDATE" ] && [ -f "$MUST_ELIMINATE_AV1_CANDIDATE" ] && is_must_eliminate_format "$src" \
