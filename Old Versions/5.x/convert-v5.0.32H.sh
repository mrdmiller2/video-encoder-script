...
get_scan_roots() {
  ...
  if [ -n "$NAME_GLOB" ]; then
    ...
    done < <(find "$SEARCH_PATH" -mindepth "$depth" -maxdepth "$depth" -type d -not -name 'ffmpeg-logs' -not -name '.*' 2>/dev/null | LC_ALL=C sort)
  ...
  # -not -name 'ffmpeg-logs' -not -name '.*': this script's own sidecar dirs
  # (ffmpeg-logs/ for per-title stderr logs, .convert-v5-filecache/ and any
  # other hidden dir for caches/flags). Without this exclusion, a shard-depth
  # scan of an already-once-processed leaf movie folder mistakes a sidecar
  # dir for the only "shard", finds 0 videos in it, and silently never scans
  # the real video file(s) sitting in the folder itself -- discovered
  # 2026-07-21 when a folder that had already produced ffmpeg-logs/ and
  # .convert-v5-filecache/ (from an earlier encode attempt) came back
  # "0 video(s)" on the next run.
  while IFS= read -r shard; do
    [ -n "$shard" ] || continue
    _roots+=("$shard")
  done < <(find "$SEARCH_PATH" -mindepth "$SHARD_DEPTH" -maxdepth "$SHARD_DEPTH" -type d -not -name 'ffmpeg-logs' -not -name '.*' 2>/dev/null | LC_ALL=C sort)
...
ffmpeg_encode() {
  ...
  run_tracked_encoder "ffmpeg encode" _run_capturing_stderr "$errfile" "${FFMPEG_CMD[@]}" "${args[@]}"
  rc=$?
  if [ "$rc" -ne 0 ]; then
    ...
    run_tracked_encoder "ffmpeg subtitle-stripped retry" _run_capturing_stderr "$errfile" "${FFMPEG_CMD[@]}" "${args[@]}"
    rc=$?
  fi
