...
validate_mkv_output() {
  ...
  if [ "$quick" != true ]; then
    ...
    validate_mkv_audio_tracks "$dst" || return 1
  fi
  return 0
}
...
audio_track_reaches_near_eof() {
  ...
  # `|| true`: this script runs under `set -o pipefail`, and `head -1`
  # deliberately closing the pipe early sends ffprobe a SIGPIPE (exit
  # 141) once it has more than one packet queued to write -- expected,
  # harmless, and NOT something `set -e` should treat as a real failure
  # here (found in team review, 2026-07-20; same fix needed anywhere a
  # `| head` follows a potentially-multi-line ffprobe/ffmpeg pipe).
  pts="$(run_ffprobe -v error -read_intervals "${start}%+${window}" -select_streams "a:${idx}" \
    -show_entries packet=pts_time -of csv=p=0 "$f" 2>/dev/null | head -1 || true)"
  [ -n "$pts" ]
}
validate_mkv_audio_tracks() {
  local dst="$1"
  local n_audio i window dur
  # wc -l (not grep -c .): grep -c exits 1 on zero matches (a legitimate,
  # common case here -- a video-only file with no audio at all) which
  # would abort the whole run under set -e. wc -l always exits 0.
  n_audio="$(run_ffprobe -v error -select_streams a -show_entries stream=index -of csv=p=0 "$dst" 2>/dev/null | wc -l | tr -d ' ')"
...
current_svtav1_major_minor() {
  if [ -z "$CURRENT_SVTAV1_MAJOR_MINOR" ]; then
    ...
    ver="$(printf '%s' "$out" | grep -i 'SVT \[version\]' | grep -oE 'v[0-9]+\.[0-9]+' | head -1)"
    ver="${ver#v}"
...
current_x265_major_minor() {
  if [ -z "$CURRENT_X265_MAJOR_MINOR" ]; then
    ...
    ver="$(printf '%s' "$out" | grep -oE 'HEVC encoder version [0-9]+\.[0-9]+' | grep -oE '[0-9]+\.[0-9]+' | head -1)"
...
try_x265_convert() {
  ...
  if [ -n "$MUST_ELIMINATE_AV1_CANDIDATE" ] && [ -f "$MUST_ELIMINATE_AV1_CANDIDATE" ] && is_must_eliminate_format "$src" \
    ...
    if awk -v d="$delta_pct" -v t="$MUST_ELIMINATE_TIE_PCT" 'BEGIN { exit !(d<=t) }'; then
      ...
      remove_output_only "$out"
      mv -f -- "$av1_cand" "$canonical_av1_out"
