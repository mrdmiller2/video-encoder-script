...
audio_track_reaches_near_eof() {
  ...
  pts="$(run_ffprobe -v error -sseof -"${window}" -select_streams "a:${idx}" \
...
validate_mkv_audio_tracks() {
  ...
  for ((i = 0; i < n_audio; i++)); do
    if ! audio_track_reaches_near_eof "$dst" "$i" "$window"; then
      warn "Validation failed: audio track a:$i has no packets in the last ${window}s of $dst -- likely a silent mid-encode audio dropout"
