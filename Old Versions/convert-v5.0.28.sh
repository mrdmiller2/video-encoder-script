...
Options:
  ...
  --profile MODE          Force encoder profile to movie|tv|anime|wanime, overriding
                          path-based auto-detection for this whole run. wanime (Western
                          flat/vector-style animation, e.g. South Park, Rick and Morty)
                          is never auto-detected -- always requires this flag.
...
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    --profile)
      case "$2" in
        movie|tv|anime|wanime) FORCE_PROFILE="$2" ;;
        *) err "--profile must be movie, tv, anime, or wanime (got: $2)"; exit 1 ;;
      esac
      shift 2 ;;
...
# Classes: movie_sdr | movie_hdr | anime_sdr | anime_hdr | tv_sdr | tv_hdr | wanime_sdr | wanime_hdr
bakeoff_profile_key() {
  local src="$1"
  local base=movie
...
load_encoder_profile() {
  local encoder="$1"
  local src="${2:-}"
  local anime=false tv=false wanime=false
  EP_PRESET=""
  EP_QUALITY=""
  EP_ENCOPTS=""
  EP_AUDIO_CODEC=""
...
# Args: codec(av1|hevc) crf src anime(true|false) hdr(true|false) hdr_mode(pq|pq_reconstruct|hlg|sdr|unknown, optional)
build_ffmpeg_video_args() {
  local codec="$1" crf="$2" src="$3" anime="$4" hdr="$5" hdr_mode="${6:-}" wanime="${7:-false}"
  ...
  case "$codec" in
    av1)
      ...
      if [ "$anime" = true ]; then
        # tune=0 scoped to anime only -- see load_encoder_profile's anime
        # branch for the full rationale (tried fleet-wide in v5.0.0, reverted
        # in v5.0.4 for live-action softness, never re-tested against anime).
        svtp="$svtp:film-grain-denoise=1:film-grain=6:enable-tf=0:enable-variance-boost=1:variance-boost-strength=3:variance-octile=4:enable-overlays=1:tune=0:sharpness=2"
      elif [ "$wanime" = true ]; then
        # Western/flat-vector animation: deliberately NOT anime's tuning --
        # see load_encoder_profile's wanime branch for the full rationale
        # (film-grain/variance-boost measurably hurt this content in real
        # fleet testing). No tune=0 here either -- flat vector art has the
        # sharpest edges of any content type, exactly where tune=0's
        # documented ringing risk is worst; sharpness alone is the safer knob.
        svtp="$svtp:sharpness=2"
      fi
      if [ "$hdr" = true ]; then
...
fixed_crf_for() {  # codec anime tv wanime hdr
  local codec="$1" anime="$2" tv="$3" wanime="$4" hdr="$5"
  case "$codec" in
    av1)
...
resolve_crf_for_encode() {  # src codec anime hdr
  ...
  if [ "$hdr" = true ] || [ "$VMAF_DISABLED" = true ] || [ "$FF_HAS_LIBVMAF" != true ]; then
    crf="$(fixed_crf_for "$codec" "$anime" "$tv" "$wanime" "$hdr")"
    [ "$hdr" = true ] && log_err "HDR source — fixed CRF $crf (VMAF unreliable on PQ/HLG)" \
                      || log_err "Fixed CRF $crf ($codec$( [ "$anime" = true ] && printf ', anime')$( [ "$wanime" = true ] && printf ', wanime')$( [ "$tv" = true ] && printf ', tv'))"
...
ffmpeg_encode() {
  ...
  local anime=false tv=false wanime=false hdr=false hdr_mode crf rc acodec abr
  ...
  rc=0
  build_ffmpeg_video_args "$codec" "$crf" "$src" "$anime" "$hdr" "$hdr_mode" "$wanime" || rc=$?
  if [ "$rc" -eq 2 ]; then
  ...
  log "ffmpeg encode ($codec crf=$crf$( [ "$hdr" = true ] && printf ', HDR10')$( [ "$anime" = true ] && printf ', anime')$( [ "$wanime" = true ] && printf ', wanime')$( [ "$tv" = true ] && printf ', tv')): $(basename "$src")"
  run_ffmpeg "${args[@]}"
  rc=$?
  if [ "$rc" -ne 0 ]; then
