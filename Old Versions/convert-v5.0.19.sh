...
resolve_encode_stage_path() {
  ...
  if [ -z "$RAMDISK_JOB_STAGE_DIR" ]; then
    printf '%s' "$dst"
    return 0
  fi
  ...
  if [ -z "$free" ] || [ "$free" -lt "$need_bytes" ]; then
    log_err "Ramdisk staging: not enough free space in $RAMDISK_JOB_DIR for this file (need ~$(( need_bytes / 1024 / 1024 ))MB) — writing directly to destination"
    printf '%s' "$dst"
    return 0
  fi
...
finalize_staged_encode_output() {
  ...
  rm -rf -- "$tmp_dir" "$staged" 2>/dev/null
  return 1
}
