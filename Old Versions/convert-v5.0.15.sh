...
ffmpeg_encode() {
  ...
  # A Dolby Vision profile 5 source has no standard PQ/HLG color_transfer tag
  # at the container level by design -- its tone curve lives entirely in the
  # proprietary RPU, not in a container-level HDR flag. Without this, hdr
  # stays false, the DoVi-detection branch in build_ffmpeg_video_args is
  # skipped entirely, and the raw base layer gets encoded with no RPU-based
  # reconstruction and no HDR10 metadata -- producing exactly the wrong-color
  # (green/red tinted) output this whole conversion path exists to prevent.
  source_has_dolby_vision "$src" && hdr=true
