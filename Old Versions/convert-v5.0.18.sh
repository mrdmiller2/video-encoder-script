...
_is_tmpfs_dir() {
  ...
  case "$PLATFORM" in
    macos)
      ...
      local dev
      dev="$(df "$d" 2>/dev/null | awk 'NR==2{print $1}')"
      [ -n "$dev" ] || return 1
...
ramdisk_discover() {
  ...
  if [ -n "$CONVERT_RAMDISK_DIR" ] && [ -d "$CONVERT_RAMDISK_DIR" ]; then
    free="$(_dir_free_bytes "$CONVERT_RAMDISK_DIR")"
    [ "${free:-0}" -ge "$need_bytes" ] && { printf '%s' "$CONVERT_RAMDISK_DIR"; return 0; }
    return 1
  fi
  while IFS= read -r d; do
    [ -d "$d" ] || continue
    _is_tmpfs_dir "$d" || continue
    free="$(_dir_free_bytes "$d")"
    [ -n "$free" ] && [ "$free" -ge "$need_bytes" ] && { printf '%s' "$d"; return 0; }
...
ramdisk_create() {
  ...
  [ -n "$avail" ] && [ "$avail" -gt 0 ] || return 1
  size_bytes=$(( avail * CONVERT_RAMDISK_PCT / 100 ))
  [ "$size_bytes" -ge "$need_bytes" ] || return 1
  ...
  case "$PLATFORM" in
    macos)
      ...
      diskutil erasevolume APFS "ConvertRAMDisk" "$dev" >/dev/null 2>&1 || { hdiutil detach "$dev" -force >/dev/null 2>&1; return 1; }
      printf '%s' "$path"
      ;;
    *)
...
# Decides where ffmpeg should actually write during encode: a staged path on
# a ramdisk if one is available/creatable and the pre-flight size estimate
# fits, else the real destination unchanged (today's direct-write behavior).
# Prints the path ffmpeg should use; the caller compares it against $2 to
# know whether a finalize/move step is needed afterward.
resolve_encode_stage_path() {
  local src="$1" dst="$2"
  local need_bytes stage_dir

  if [ "$CONVERT_NO_RAMDISK" = true ]; then
    printf '%s' "$dst"
    return 0
  fi

  need_bytes="$(file_size_bytes "$src")"
  [ "$need_bytes" -gt 0 ] || { printf '%s' "$dst"; return 0; }
  need_bytes=$(( need_bytes + need_bytes * RAMDISK_SIZE_ESTIMATE_MARGIN_PCT / 100 ))

  stage_dir="$(ramdisk_discover "$need_bytes")" || stage_dir="$(ramdisk_create "$need_bytes")"
  if [ -z "$stage_dir" ]; then
    log_err "Ramdisk staging: no suitable RAM-backed target (need ~$(( need_bytes / 1024 / 1024 ))MB) — writing directly to destination"
    printf '%s' "$dst"
    return 0
  fi

  log_err "Ramdisk staging: using $stage_dir (estimated output ~$(( need_bytes / 1024 / 1024 ))MB)"
  printf '%s/.convert-stage.%s.%s' "$stage_dir" "$$" "$(basename "$dst")"
}
...
finalize_staged_encode_output() {
  ...
  [ "$staged" = "$final_dst" ] && return 0
  [ -s "$staged" ] || return 1

  local tmp_on_dst
  tmp_on_dst="$(mktemp "${final_dst}.stageXXXXXX")" || return 1
  if cp "$staged" "$tmp_on_dst" && [ "$(file_size_bytes "$tmp_on_dst")" -eq "$(file_size_bytes "$staged")" ]; then
    mv -f "$tmp_on_dst" "$final_dst" && _restore_default_file_mode "$final_dst"
    rm -f "$staged"
    return 0
  fi
  rm -f "$tmp_on_dst" "$staged" 2>/dev/null
  return 1
}
...
ffmpeg_encode() {
  ...
  local anime=false hdr=false hdr_mode crf rc acodec abr
