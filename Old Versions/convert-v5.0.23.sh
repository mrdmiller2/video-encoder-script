...
# a normal `>`-created file would have been. Restore the umask-derived
# default mode right after the swap so the temp file's identity doesn't leak
# into the permissions of the file it replaced.
...
build_shard_snapshot() {
  ...
  mv -f "$tmpf" "$out_file" 2>/dev/null || rm -f "$tmpf" 2>/dev/null
}
...
convert_scan_producer() {
  ...
  scan_total_tmp="$(mktemp "${CONVERT_SCAN_TOTAL_FILE}.XXXXXX" 2>/dev/null)" && {
    ...
    mv -f "$scan_total_tmp" "$CONVERT_SCAN_TOTAL_FILE" 2>/dev/null || rm -f "$scan_total_tmp"
  }
