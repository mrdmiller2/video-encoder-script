#!/usr/bin/env bash
# convert-v4.0.49.sh — Organize movie folders, transcode TV/movies to AV1/x265 MKV.
# Version: 4.0.49
...
#   Sources shorter than 720p are upscaled to 1920x1080 — size growth is expected; do not
#     reject upscaled encodes solely for exceeding the source byte size (keep smaller of AV1/x265).
...
size_keep_policy_av1() {
  ...
  # Upscale intentionally adds pixels — byte growth vs source is expected.
...
try_av1_convert() {
  ...
  if [ "$policy" = keep ]; then
    ...
    if [ "$upscaled" = true ] && [ "$new_sz" -gt "$orig_sz" ]; then
      log "Kept AV1 ($(awk -v o="$orig_sz" -v n="$new_sz" 'BEGIN { printf "%.1f%% of original", (n/o)*100 }'); 1080p upscale — size growth expected): $out"
    else
...
try_x265_convert() {
  ...
  if [ "$policy" = keep ]; then
    finalize_mkv_output "$out" "$src" "$title"
    if [ "$upscaled" = true ] && [ "$new_sz" -gt "$orig_sz" ]; then
      log "Kept x265 ($(awk -v o="$orig_sz" -v n="$new_sz" 'BEGIN { printf "%.1f%% of original", (n/o)*100 }'); 1080p upscale — size growth expected): $out"
    else
