#!/usr/bin/env bash
# convert-v4.0.21.sh — Organize movie folders, transcode TV/movies to AV1/x265 MKV.
# Version: 4.0.21
...
warn_if_root_wsl() {
  ...
  if [ "$(id -u)" -eq 0 ]; then
    warn "Running as root (sudo) on WSL — Windows HandBrake .exe and NFS log writes often fail"
    warn "Run without sudo: ./convert-v4.0.21.sh -p ... --handbrake '/mnt/c/Program Files/HandBrake/HandBrakeCLI.exe'"
    if [ -n "${SUDO_USER:-}" ]; then
      warn "Or: sudo -u \"$SUDO_USER\" $0 ..."
    fi
  fi
}
...
_handbrake_reports_nvenc() {
  ...
  "$candidate" --help 2>&1 | grep -qiE 'nvenc: version [0-9]|nvenc_av1_10bit|nvenc_h265'
}
...
_detect_nvidia_via_handbrake() {
  ...
  if ! grep -qiE 'nvenc: version [0-9]|nvenc_av1_10bit|nvenc_h265' <<<"$hb_help"; then
    return 1
  fi

  HAS_NVIDIA=true
  NVIDIA_GPU_COUNT=1
  if grep -qiE 'nvdec: is available|Use .nvdec. to enable' <<<"$hb_help"; then
    HAS_HW_DECODE=true
    HW_DECODE_NAME=nvdec
...
detect_nvenc_av1_tune() {
  ...
  if command -v timeout >/dev/null 2>&1; then
    ...
    "${HANDBRAKE_CMD[@]}" "${hb_args[@]}" >"$probe_log" 2>&1
  fi
  rc=$?
  set -e
  ...
  if [ "$rc" -eq 124 ]; then
    ...
    ! grep -qiE 'Unable to parse option value "uhq"|Error setting option tune|encavcodecInit: avcodec_open failed' "$probe_log"; then
    NVENC_AV1_TUNE=uhq
...
MASTER_LOG_FILE="$JOB_ROOT/convert-v4.log"
...
resume_init_paths() {
  RESUME_STATE_FILE="$JOB_ROOT/convert-v4.state"
  RESUME_QUEUE_FILE="$JOB_ROOT/convert-v4.queue"
  RESUME_SHARDS_FILE="$JOB_ROOT/convert-v4.shards"
}
...
init_stats_log() {
  ...
  if ! {
    ...
    if [ -f "$JOB_ROOT/convert-v4.state" ] && [ "$NO_RESUME" = false ]; then
      echo "resume: convert-v4.state present (auto-resume on convert)"
    fi
  } >>"$MASTER_LOG_FILE" 2>/dev/null; then
    fallback="/tmp/convert-v4-$(id -un 2>/dev/null || echo user).log"
    MASTER_LOG_FILE="$fallback"
    STATS_LOG_FILE="$MASTER_LOG_FILE"
    warn "Cannot write master log under $JOB_ROOT — using $MASTER_LOG_FILE"
    {
      echo ""
      echo "=== $SCRIPT_NAME v$VERSION — $ts ==="
      echo "job_root: $JOB_ROOT"
      echo "path: $SEARCH_PATH"
      echo "dry_run: $DRY_RUN | organize: $DO_ORGANIZE | convert: $DO_CONVERT | skip_av1: $SKIP_AV1 | skip_x265: $SKIP_X265 | nvidia: $HAS_NVIDIA"
    } >>"$MASTER_LOG_FILE" 2>/dev/null || warn "Master log disabled (job root not writable?)"
  fi
  set +e
...
end_shard_log() {
  ...
  cat "$SHARD_LOG_FILE" >>"$MASTER_LOG_FILE"
...
source_has_dolby_vision() {
  ...
  run_ffprobe -v error -select_streams v:0 -show_entries stream_side_data=side_data_type \
    -of csv=p=0 "$src" 2>/dev/null | rg -qi 'DOVI configuration record'
}
...
main() {
  ...
  log "Master log: $JOB_ROOT/convert-v4.log (tail -f here for full job status)"
