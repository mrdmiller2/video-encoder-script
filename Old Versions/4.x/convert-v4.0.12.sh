#!/usr/bin/env bash
# convert-v4.0.12.sh — Organize movie folders, transcode TV/movies to AV1/x265 MKV.
# Version: 4.0.12
  ...
  if [ "$anime" = true ]; then
    case "$encoder" in
      ...
      nvenc_av1_10bit)
        EP_PRESET=slowest
        EP_QUALITY=32
        EP_ENCOPTS='spatial-aq=1:temporal-aq=1:rc-lookahead=32:tune=hq'
        EP_VIDEO_FILTERS=(--lapsharp=light)
        EP_AUDIO_CODEC=opus
        EP_AUDIO_BITRATE=$OPUS_BITRATE
        EP_HW_DECODE=true
  ...
  if [ "$HAS_NVIDIA" = true ]; then
    ...
    if [ -n "$hb_title" ]; then
      CUDA_VISIBLE_DEVICES="$GPU_AV1" run_handbrake_with_progress "Bake-off: nvenc_av1_10bit" -i "$sample_src" -t "$hb_title" -o "$nvenc_out" -f mkv \
        ...
        -D "$AUDIO_DRC" --gain "$AUDIO_GAIN" \
        "${EP_VIDEO_FILTERS[@]}" || warn "Bake-off: nvenc_av1_10bit sample encode failed"
    else
    ...
    log "Bake-off SSIM — nvenc_av1_10bit: ${ss_nv:-0} | svt_av1_10bit: ${ss_svt:-0}"
