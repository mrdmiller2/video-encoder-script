#!/usr/bin/env bash
# convert-v4.0.32.sh — Organize movie folders, transcode TV/movies to AV1/x265 MKV.
# Version: 4.0.32
...
bakeoff_encoder_for_src() {
  ...
  if _path_on_cifs "$src" && [ "$USE_NVIDIA_ENCODE" = false ] && [ "$USE_QSV_ENCODE" = false ]; then
    bakeoff_default_av1_encoder
    BAKEOFF_ENCODER_CHOICE[$key]="$AV1_ENCODER"
    log "Bake-off skipped (CIFS + software encode) — profile '$key' locked: $AV1_ENCODER"
    warn "Software encode over SMB is very slow — use WSL + Windows HandBrake (NVENC) or --skip-bakeoff"
    return 0
  fi
...
convert_pipeline_resume_offset() {
  ...
  log "Resume: last job was $RESUME_LAST_STATUS on $(basename "$RESUME_LAST_SOURCE") (shard: ${RESUME_LAST_SHARD:-unknown})"
  log "Resume: skipping first $resume_idx queued item(s)"
  printf '%s' "$resume_idx"
}
...
convert_run_pipeline_jobs() {
  ...
  while true; do
    ...
    while [ "${#batch[@]}" -lt "$ENCODE_INSPECT_BATCH_SIZE" ]; do
      ...
      if [ "$CONVERT_READY_OFFSET" -le "$resume_skip" ]; then
        pending="$(convert_pipeline_ready_pending)"
    ...
    for bf in "${batch[@]}"; do
      ...
      if [ -f "$CONVERT_SCAN_DONE_FILE" ] && [ -n "${CONVERT_JOB_TOTAL:-}" ] && [ "$CONVERT_JOB_TOTAL" -gt 0 ]; then
