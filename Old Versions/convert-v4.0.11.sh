#!/usr/bin/env bash
# convert-v4.0.11.sh — Organize movie folders, transcode TV/movies to AV1/x265 MKV.
# Version: 4.0.11
...
resume_init_paths() {
  RESUME_STATE_FILE="$SEARCH_PATH/convert-v4.state"
  RESUME_QUEUE_FILE="$SEARCH_PATH/convert-v4.queue"
  RESUME_SHARDS_FILE="$SEARCH_PATH/convert-v4.shards"
}
...
init_stats_log() {
  ...
  ts="$(date -u '+%Y-%m-%d %H:%M:%S UTC')"
  {
    ...
    if [ -f "$SEARCH_PATH/convert-v4.state" ] && [ "$NO_RESUME" = false ]; then
...
bakeoff_default_av1_encoder() {
  if [ "$HAS_NVIDIA" = true ]; then
    ...
    warn "Bake-off incomplete — defaulting to $AV1_ENCODER (nvenc_av1 sample failed)"
  else
    AV1_ENCODER="svt_av1_10bit"
...
pick_av1_encoder() {
  ...
  log "One-time encoder bake-off (${SAMPLE_SECONDS}s sample) — comparing AV1 encoders before queue processing"
  ...
  if [ -n "$hb_title" ]; then
    ...
    run_handbrake_with_progress "Bake-off: svt_av1_10bit" -i "$sample_src" -t "$hb_title" -o "$svt_out" -f mkv \
      ...
      "${EP_VIDEO_FILTERS[@]}" || warn "Bake-off: svt_av1_10bit sample encode failed"
  else
  ...
  if [ "$HAS_NVIDIA" = true ]; then
    ...
    if [ -n "$hb_title" ]; then
      CUDA_VISIBLE_DEVICES="$GPU_AV1" run_handbrake_with_progress "Bake-off: nvenc_av1" -i "$sample_src" -t "$hb_title" -o "$nvenc_out" -f mkv \
        --start-at "duration:$start" --stop-at "duration:$end" \
        ...
        -D "$AUDIO_DRC" --gain "$AUDIO_GAIN" \
        "${EP_VIDEO_FILTERS[@]}" || warn "Bake-off: nvenc_av1 sample encode failed"
    else
      CUDA_VISIBLE_DEVICES="$GPU_AV1" run_handbrake_with_progress "Bake-off: nvenc_av1" -i "$clip" -o "$nvenc_out" -f mkv \
        -e nvenc_av1 -q "$EP_QUALITY" --encoder-preset "$EP_PRESET" \
        --encopts "$EP_ENCOPTS" --enable-hw-decoding "${HW_DECODE_NAME:-nvdec}" \
        --all-audio --aencoder "$EP_AUDIO_CODEC" -B "$EP_AUDIO_BITRATE" \
        -D "$AUDIO_DRC" --gain "$AUDIO_GAIN" \
