...
resolve_encode_stage_path() {
  ...
  local_dir="$(_local_stage_dir_for "$dst")" || {
    warn "Could not create a private staging directory next to $dst — encoding directly to the final path (symlink-race window applies)"
    printf '%s' "$dst"
    return 0
  }
...
convert_init_pipeline_files() {
  ...
  # Persistent write fd, opened here (before convert_scan_producer forks)
  # rather than reopening CONVERT_READY_FILE by path on every discovered
  # item -- the forked producer inherits this fd, and O_APPEND writes
  # through an already-open fd are immune to the path later being swapped
  # for a symlink, unlike reopening by name on each append.
  exec {CONVERT_READY_WRITE_FD}>>"$CONVERT_READY_FILE"
}
