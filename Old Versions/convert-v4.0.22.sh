#!/usr/bin/env bash
# convert-v4.0.22.sh — Organize movie folders, transcode TV/movies to AV1/x265 MKV.
# Version: 4.0.22
...
_handbrake_reports_nvenc() {
  ...
  if [ "$(id -u)" -eq 0 ] && [ -n "${SUDO_USER:-}" ] && [ "$PLATFORM" = wsl ] && [[ "$candidate" == *.exe ]]; then
    sudo -u "$SUDO_USER" -H -- "$candidate" --help 2>&1 | grep -qiE 'nvenc: version [0-9]|nvenc_av1_10bit|nvenc_h265'
  else
