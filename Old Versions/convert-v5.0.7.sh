#!/usr/bin/env bash
# convert-v5.0.7.sh — Organize movie folders, transcode TV/movies to AV1/x265 MKV.
# Version: 5.0.7
#
...
#      empty .AV1.mkv/.x265.mkv/.merged.mkv, stale IN_PROGRESS flags with no
#      live process, and orphaned .merged.mkv whose source parts are gone.
#      Report-only by default; --clean-junk-apply deletes.
...
#   Sources shorter than 720p are upscaled to 1920x1080 — size growth is expected up to
...
Options:
  ...
  --clean-junk            Scan --path for junk (zero-byte outputs, stale IN_PROGRESS flags,
                          orphaned .merged.mkv) and report findings only, then exit
...
source_triggers_1080p_upscale() {
  ...
  [ -n "$h" ] || return 1
  awk -v h="$h" 'BEGIN { exit !(h + 0 > 0 && h + 0 < 720) }'
}
...
filecache_get() {  # dir -> file list on stdout; returns 1 on cache miss
  ...
  [ -f "$cache" ] || return 1
  mtime="$(mkv_structure_stat_key "$dir" 2>/dev/null)"; mtime="${mtime##*|}"
  [ -n "$mtime" ] || return 1
...
# ============================================================================
# Finds leftover debris from interrupted runs and superseded merges: zero-byte
# or structurally-invalid .AV1.mkv/.x265.mkv/.merged.mkv, stale IN_PROGRESS
# flags whose process is gone, and .merged.mkv files whose source parts no
# longer exist. Report-only by default; --clean-junk-apply actually deletes.
# Never touches original sources or the human-review logs (bad_sources.txt,
...
folder_marked_done() {  # dir -> 0 if a valid (non-stale) done-flag exists
  ...
  dir_mtime="$(mkv_structure_stat_key "$dir" 2>/dev/null)"; dir_mtime="${dir_mtime##*|}"
...
clean_junk_scan() {
  local root="$1"
  local -a zero_byte=() stale_flags=() orphan_merges=()
  local f dir title parts
  ...
  while IFS= read -r f; do
    [ -n "$f" ] || continue
    dir="$(dirname "$f")"
    title="$(basename "$f" .mkv)"; title="${title%.merged}"
    local -a groups=()
    detect_multipart_groups "$dir" groups
    local still_has_parts=false g
    for g in "${groups[@]}"; do
      [ "${g%%$'\t'*}" = "$title" ] && still_has_parts=true && break
    done
    [ "$still_has_parts" = true ] || orphan_merges+=("$f")
  done < <(find "$root" -type f -iname '*.merged.mkv' 2>/dev/null)

  local total=$(( ${#zero_byte[@]} + ${#stale_flags[@]} + ${#orphan_merges[@]} ))
  if [ "$total" -eq 0 ]; then
  ...
  [ "${#orphan_merges[@]}" -gt 0 ] && { log "  Orphaned .merged.mkv, source parts gone (${#orphan_merges[@]}):"; printf '    %s\n' "${orphan_merges[@]}"; }
  ...
  for f in "${zero_byte[@]}" "${stale_flags[@]}" "${orphan_merges[@]}"; do
