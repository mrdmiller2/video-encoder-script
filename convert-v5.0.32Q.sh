...
current_tool_versions_tag_suffix() {
  if [ -z "$TOOL_VERSIONS_TAG_SUFFIX" ]; then
    local ffv mkvv
    ffv="$("${FFMPEG_CMD[@]}" -version 2>&1 | head -1 | awk '{print $3}')"
    mkvv="$(run_mkvmerge --version 2>&1 | awk '{print $2}' | tr -d 'v')"
    TOOL_VERSIONS_TAG_SUFFIX=" [tools: ffmpeg=${ffv:-unknown};svtav1=$(current_svtav1_major_minor);x265=$(current_x265_major_minor);mkvmerge=${mkvv:-unknown}]"
  fi
