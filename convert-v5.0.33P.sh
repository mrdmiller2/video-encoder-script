...
subtitle_stream_has_real_content() {
  local src="$1" idx="$2" codec pkt_count decode_out

  pkt_count="$(run_ffprobe -v error -select_streams "s:$idx" -show_entries packet=pts_time \
    -of csv=p=0 "$src" 2>/dev/null | grep -c .)" || pkt_count=0
  [ "${pkt_count:-0}" -gt 0 ] || return 1
  ...
  case "$codec" in
    subrip|srt|ass|ssa|mov_text|webvtt|text)
      # Decode to SRT and strip cue numbers/timing lines/blank lines,
      # then ASS override blocks ({\an5}, {\pos(...)}, etc.) and any
      # <tag>/</tag> wrapper ffmpeg's srt encoder adds for styled ASS
      # cues -- anything left over is real text. A track can have
      # packets but still be functionally empty (e.g. every cue authored
      # as "" or as position-only override codes with no dialogue) --
      # found via direct testing (2026-08-02): an ASS cue containing only
      # "{\an5}" survived the original cue-number/timing-only strip as
    ...
    *)
      ...
      # can't be text-stripped -- packet count plus a decode-error grep is
      # the best available proxy for "actually renderable".
      run_ffmpeg_validation -v error -i "$src" -map "0:s:$idx" -f null - 2>&1 \
        | grep -qiE 'error|invalid' && return 1
      return 0
      ;;
  esac
}
...
build_real_subtitle_map_args() {
  ...
  REAL_SUBTITLE_MAP_ARGS=()
  subs_list="$(run_ffprobe -v error -select_streams s -show_entries stream=index -of csv=p=0 "$src" 2>/dev/null)"
  n_subs="$(printf '%s\n' "$subs_list" | grep -c . || true)"
  [ "${n_subs:-0}" -gt 0 ] || return 0
