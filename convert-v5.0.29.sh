...
SKIP_AV1=false
SKIP_X265=false
# Override for movie/tv/anime/wanime auto-detection (--profile). Empty =
# auto (path-based detection, movie/tv/anime only -- wanime is never auto-
# detected, see below). Needed for mixed-content library folders where the
# path alone can't tell the difference -- e.g. Western animation (South
# Park, Rick and Morty, ...) sitting inside a live-action /Television/
# American/ folder, which would otherwise get the plain TV profile.
# wanime, not anime: real fleet testing found the anime profile's tuning
# (built for Japanese hand-drawn line art) actively hurts Western flat/
# vector-style animation -- a real episode came out LARGER than the source.
FORCE_PROFILE=""
SHARD_DEPTH=1
NO_SHARD=false
...
VMAF_TARGET_TV="${CONVERT_VMAF_TARGET_TV:-94.0}"  # 1080p SDR TV shows (non-anime) — independently tunable, same default as movie
VMAF_TARGET_WANIME="${CONVERT_VMAF_TARGET_WANIME:-94.0}"  # Western animation (--profile wanime only) — independently tunable
VMAF_TARGET_VINTAGE="${CONVERT_VMAF_TARGET_VINTAGE:-94.0}"  # Old/grainy live-action masters (--profile vintage only)
...
Options:
  ...
  --profile MODE          Force encoder profile to movie|tv|anime|wanime|vintage,
                          overriding path-based auto-detection for this whole run.
                          wanime (Western flat/vector-style animation, e.g. South
                          Park, Rick and Morty) and vintage (old/grainy live-action
                          masters) are never auto-detected -- always require this flag.
  ...
  --vmaf-target-tv N      VMAF floor for non-anime TV shows only (default 94.0)
...
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    --profile)
      case "$2" in
        movie|tv|anime|wanime|vintage) FORCE_PROFILE="$2" ;;
        *) err "--profile must be movie, tv, anime, wanime, or vintage (got: $2)"; exit 1 ;;
      esac
      shift 2 ;;
    ...
    --vmaf-target) VMAF_TARGET_MOVIE="$2"; VMAF_TARGET_ANIME="$2"; VMAF_TARGET_TV="$2"; shift 2 ;;
    --vmaf-target-tv) VMAF_TARGET_TV="$2"; shift 2 ;;
...
is_tv_library_path() {
  local p="$1"
  case "$p" in
    */Television/*|*/Television|*/TV/*|*/TV|*/Anime/*|*/Anime|*/TV\ Shows/*|*/TV\ Shows|*/Series/*|*/Series)
      return 0
      ;;
  esac
  return 1
...
uses_anime_profile() {
  local src="${1:-}"
  # --profile overrides auto-detection entirely for the whole run -- needed
  # for mixed-content folders the path alone can't disambiguate (e.g. adult
  # animation sitting inside a live-action /Television/American/ folder).
  if [ -n "$FORCE_PROFILE" ]; then
    if [ "$FORCE_PROFILE" = anime ]; then return 0; else return 1; fi
  fi
...
# Non-anime TV shows get their own profile, distinct from theatrical movies.
# Path-based only (like uses_anime_profile), not filename episode-pattern
# based (is_tv_episode) -- that heuristic exists for naming/organization and
# has false-positive shapes (e.g. a movie title ending "... 2") that are fine
# for cosmetic logging but too risky to drive actual encode tuning decisions.
# Anime always takes precedence: is_tv_library_path also matches /Anime/
# paths (anime libraries are physically laid out the same way), so this must
# run only after uses_anime_profile has already ruled anime out.
uses_tv_profile() {
  local src="${1:-}"
  if [ -n "$FORCE_PROFILE" ]; then
    if [ "$FORCE_PROFILE" = tv ]; then return 0; else return 1; fi
  fi
  uses_anime_profile "$src" && return 1
  is_tv_library_path "$src" && return 0
  is_tv_library_path "$SEARCH_PATH" && return 0
  return 1
}

# Western animation (flat/vector-style digital ink-and-paint -- South Park,
# Rick and Morty, etc.) is visually a different content class from Japanese
# hand-drawn anime: real fleet testing found the anime profile's film-grain
# synthesis and variance-boost tuning (built for hand-drawn line art) hurt
# compression on this content badly enough that a real episode came out
# LARGER than its source. Deliberately has NO path-based auto-detection --
# unlike anime/tv, this content lives mixed inside ordinary /Television/
# folders with no reliable folder-naming convention to key off, the same
# reasoning that motivated --profile in the first place. --profile wanime
# is the only way to select it, ever.
uses_wanime_profile() {
  [ "$FORCE_PROFILE" = wanime ]
}

# Old/grainy live-action masters (film scans, older TV masters with real
# photochemical grain). Like wanime, manual-only -- "old/grainy" isn't
# reliably inferable from a folder path. --profile vintage is the only way
# to select it, ever.
uses_vintage_profile() {
  [ "$FORCE_PROFILE" = vintage ]
}
...
# Caught this exact bug via a real end-to-end test on Crystalight (macOS)
...
# Classes: movie_sdr | movie_hdr | anime_sdr | anime_hdr | tv_sdr | tv_hdr | wanime_sdr | wanime_hdr | vintage_sdr | vintage_hdr
bakeoff_profile_key() {
  local src="$1"
  local base=movie
  if uses_anime_profile "$src"; then
    base=anime
  elif uses_wanime_profile; then
    base=wanime
  elif uses_vintage_profile; then
    base=vintage
  elif uses_tv_profile "$src"; then
    base=tv
  fi
...
load_encoder_profile() {
  local encoder="$1"
  local src="${2:-}"
  local anime=false tv=false wanime=false vintage=false
  EP_PRESET=""
  EP_QUALITY=""
  EP_ENCOPTS=""
  EP_AUDIO_CODEC=""
  ...
  if [ -n "$src" ] && uses_anime_profile "$src"; then
    ...
    wanime=true
    EP_PROFILE_NAME=wanime
  elif uses_vintage_profile; then
    vintage=true
    EP_PROFILE_NAME=vintage
  elif [ -n "$src" ] && uses_tv_profile "$src"; then
    tv=true
    EP_PROFILE_NAME=tv
  fi
  ...
  if [ "$anime" = true ]; then
    case "$encoder" in
      svt_av1_10bit)
        ...
        # tune=0 (perceptual/VQ) was tried fleet-wide in v5.0.0 and reverted
        # in v5.0.4 after real playback A/B testing found it softer than
        # SVT-AV1's PSNR-oriented default -- but that finding was against
        # live-action TV content, never re-validated against anime's flat-
        # color/line-art visual character, where community consensus favors
        # tune=0 specifically for line-art sharpness. Scoped to anime only;
        # movie/tv keep the default (unchanged, still validated for live-action).
        # Caveat (unverified against our own content, per a secondary source
        # rather than SVT-AV1's own docs): tune=0's psycho-visual optimizer
        # can ring around strong edges in flat-color art. Not hedged against
        # here -- needs a real playback A/B test to judge, not a guess.
        # film-grain=6 (not 12): SVT-AV1's own guidance is 4-6 for 2D
        # animation; 12 was applying denoising/synthesis more aggressive than
        # documented for this content type.
        # sharpness=2 (range -7..7, default 0, per SVT-AV1's own docs): a
        # more surgical, lower-risk knob than tune=0 for line-art crispness,
        # added alongside rather than instead of tune=0. (Note: tune=2 is
        # SSIM-metric tuning, not an "animation" mode as sometimes claimed
        # online, and tune=3/"animation" only exists in the SVT-AV1-PSY fork,
        # not the mainline build this fleet runs -- verified against
        # SVT-AV1's own Parameters.md, not a secondary summary.)
        EP_ENCOPTS='enable-qm=1:film-grain-denoise=1:film-grain=6:qm-min=0:scd=1:enable-tf=0:keyint=15s:aq-mode=2:enable-variance-boost=1:variance-boost-strength=3:variance-octile=4:enable-overlays=1:tune=0:sharpness=2'
  ...
  # Western animation (flat/vector digital ink-and-paint -- South Park, Rick
  # and Morty, etc.): deliberately NOT the anime branch's tuning. Real fleet
  # testing found anime's film-grain synthesis + variance-boost made this
  # content WORSE (a real episode came out larger than its source) -- this
  # content has essentially no real grain to match and few of the smooth
  # gradients variance-boost exists to protect. Movie/tv's plain ENCOPTS
  # (film-grain implicitly off, since it's never set) already matches
  # community guidance of film-grain=0 for this content class. sharpness=2
  # (not tune=0): flat vector art has the sharpest, highest-contrast edges
  # of any content type here, which is exactly where tune=0's documented
  # ringing-artifact risk is most acute -- sharpness is a more surgical,
  # lower-risk knob for the same line-art-crispness goal.
  if [ "$wanime" = true ]; then
    case "$encoder" in
      svt_av1_10bit)
        EP_PRESET=8
        EP_QUALITY=$SVT_AV1_CQ_WANIME
        EP_ENCOPTS='enable-qm=1:qm-min=0:keyint=15s:scd=1:aq-mode=2:sharpness=2'
        EP_AUDIO_CODEC=opus
        EP_AUDIO_BITRATE=$OPUS_BITRATE
        EP_HW_DECODE=auto
        ;;
      nvenc_av1_10bit)
        EP_PRESET=slowest
        EP_QUALITY=$NVENC_AV1_CQ_WANIME
        EP_ENCOPTS="$(nvenc_av1_encopts)"
        EP_AUDIO_CODEC=opus
        EP_AUDIO_BITRATE=$OPUS_BITRATE
        EP_HW_DECODE=true
  ...
  # Vintage: old/grainy live-action masters (film scans, older TV masters
  # with real photochemical grain). Unlike movie/tv/wanime, film-grain
  # synthesis is deliberately re-enabled -- research and community guidance
  # (Netflix/streaming-industry practice) is that grain synthesis on genuinely
  # grainy sources can save up to ~50% bitrate vs re-encoding real grain as
  # detail, since the encoder denoises to a clean base layer and lets the
  # decoder regenerate matching-looking grain. film-grain=12 is a single
  # reasonable starting point for typical 35mm-scan grain -- per SVT-AV1's
  # own guidance there's no universal correct value; heavier 16mm/high-ISO
  # scans may want 14-16, cleaner masters 8. enable-tf=1 (unlike anime's tf=0):
  # temporal filtering is a real efficiency win for live-action motion and
  # doesn't have anime's hand-drawn-frame smearing risk. sharpness=1 (not 2)
  # and variance-boost-strength=2 (not 3): kept deliberately lighter than
  # anime's values per two independent external reviews (both
  # consulted this session) that flagged anime's sharpness=2 as a ringing
  # risk on line art and recommended lighter defaults for grain-heavy content
  # generally -- untested against real vintage content as of this writing.
  if [ "$vintage" = true ]; then
    case "$encoder" in
      svt_av1_10bit)
        EP_PRESET=5
        EP_QUALITY=$SVT_AV1_CQ_VINTAGE
        EP_ENCOPTS='enable-qm=1:film-grain-denoise=1:film-grain=12:qm-min=0:scd=1:enable-tf=1:keyint=15s:aq-mode=2:enable-variance-boost=1:variance-boost-strength=2:variance-octile=4:tune=0:sharpness=1'
        EP_AUDIO_CODEC=opus
        EP_AUDIO_BITRATE=$OPUS_BITRATE
        EP_HW_DECODE=auto
        ;;
      nvenc_av1_10bit)
        EP_PRESET=slowest
        EP_QUALITY=$NVENC_AV1_CQ_VINTAGE
        EP_ENCOPTS="$(nvenc_av1_encopts)"
        EP_AUDIO_CODEC=opus
        EP_AUDIO_BITRATE=$OPUS_BITRATE
        EP_HW_DECODE=true
        ;;
      x265)
        # --tune grain is a real x265 tune (confirmed against x265's own
        # docs): adjusts ratecontrol/AQ behavior specifically to avoid grain
        # strobing. psy-rd/psy-rdoq raised from movie/tv's baseline per
        # community guidance for light/medium film grain (2.0-5.0 range;
        # heavy grain sources may want 10-15, not defaulted here).
        EP_PRESET=medium
        EP_QUALITY=22
        EP_ENCOPTS='tune=grain:keyint=240:min-keyint=24:bframes=6:ref=4:rc-lookahead=30:psy-rd=2.0:psy-rdoq=3.0'
        EP_AUDIO_CODEC=aac
        EP_AUDIO_BITRATE=$AAC_BITRATE
        EP_HW_DECODE=false
        ;;
  ...
  # Non-anime TV shows: own named profile so it's independently tunable, but
  # deliberately identical to the movie branch below (same ENCOPTS/PRESET,
  # only the quality constants differ, and only for the two primary AV1
  # paths) -- there's no established technical rationale to diverge live-
  # action TV's psy/aq tuning from movies the way anime's flat-color content
  # justifies tune=animation/film-grain synthesis.
  if [ "$tv" = true ]; then
    case "$encoder" in
      svt_av1_10bit)
        EP_PRESET=8
        EP_QUALITY=$SVT_AV1_CQ_TV
...
build_handbrake_args() {
  ...
  if [ "$EP_PROFILE_NAME" != movie ]; then
    log "Encoder profile: $EP_PROFILE_NAME ($encoder)"
  fi
...
svtav1_profile_extras() {  # anime wanime vintage
  local anime="$1" wanime="${2:-false}" vintage="${3:-false}"
  if [ "$anime" = true ]; then
    # tune=0 scoped to anime only -- see load_encoder_profile's anime branch
    # for the full rationale (tried fleet-wide in v5.0.0, reverted in v5.0.4
    # for live-action softness, never re-tested against anime).
    printf '%s' 'film-grain-denoise=1:film-grain=6:enable-tf=0:enable-variance-boost=1:variance-boost-strength=3:variance-octile=4:enable-overlays=1:tune=0:sharpness=2'
  elif [ "$wanime" = true ]; then
    # Western/flat-vector animation: deliberately NOT anime's tuning -- see
    # load_encoder_profile's wanime branch for the full rationale (film-grain/
    # variance-boost measurably hurt this content in real fleet testing). No
    # tune=0 here either -- flat vector art has the sharpest edges of any
    # content type, exactly where tune=0's documented ringing risk is worst;
    # sharpness alone is the safer knob.
    printf '%s' 'sharpness=2'
  elif [ "$vintage" = true ]; then
    # Old/grainy live-action masters -- see load_encoder_profile's vintage
    # branch for the full rationale.
    printf '%s' 'film-grain-denoise=1:film-grain=12:enable-tf=1:enable-variance-boost=1:variance-boost-strength=2:variance-octile=4:tune=0:sharpness=1'
  fi
}
...
svtav1_profile_uses_grain_synthesis() {  # anime vintage
  local anime="$1" vintage="${2:-false}"
  [ "$anime" = true ] || [ "$vintage" = true ]
}

# Sets FF_VIDEO_ARGS (array) for the chosen codec/quality/profile.
# Args: codec(av1|hevc) crf src anime(true|false) hdr(true|false) hdr_mode(pq|pq_reconstruct|hlg|sdr|unknown, optional) wanime(true|false) vintage(true|false)
build_ffmpeg_video_args() {
  local codec="$1" crf="$2" src="$3" anime="$4" hdr="$5" hdr_mode="${6:-}" wanime="${7:-false}" vintage="${8:-false}"
  ...
  case "$codec" in
    av1)
      ...
      local _extras
      _extras="$(svtav1_profile_extras "$anime" "$wanime" "$vintage")"
      [ -n "$_extras" ] && svtp="$svtp:$_extras"
      if [ "$hdr" = true ]; then
    ...
    hevc)
      ...
      if [ "$anime" = true ]; then
        ...
        # tune=grain is a real x265 tune (confirmed against x265's own docs):
        # adjusts ratecontrol/AQ behavior specifically to avoid grain
        # strobing. psy-rd/psy-rdoq raised from the movie/tv baseline per
        # community guidance for light/medium film grain.
        x265p="log-level=error:tune=grain:keyint=240:min-keyint=24:bframes=6:ref=4:rc-lookahead=30:psy-rd=2.0:psy-rdoq=3.0"
      fi
      if [ "$hdr" = true ]; then
        if [ "$hdr_mode" = hlg ]; then
...
vmaf_target_for_source() {
  ...
  if uses_wanime_profile; then printf '%s' "$VMAF_TARGET_WANIME"; return; fi
  if uses_vintage_profile; then printf '%s' "$VMAF_TARGET_VINTAGE"; return; fi
  h="$(video_height "$src")"
  if [ "${h:-0}" -gt 1600 ]; then printf '%s' "$VMAF_TARGET_4K"; return; fi
  if uses_tv_profile "$src"; then printf '%s' "$VMAF_TARGET_TV"; return; fi
  printf '%s' "$VMAF_TARGET_MOVIE"
}

fixed_crf_for() {  # codec anime tv wanime vintage hdr
  local codec="$1" anime="$2" tv="$3" wanime="$4" vintage="$5" hdr="$6"
  case "$codec" in
    av1)
      [ "$hdr" = true ]   && { printf '%s' "$FIXED_CRF_SVT_HDR"; return; }
      [ "$anime" = true ] && { printf '%s' "$FIXED_CRF_SVT_ANIME"; return; }
      [ "$wanime" = true ] && { printf '%s' "$FIXED_CRF_SVT_WANIME"; return; }
      [ "$vintage" = true ] && { printf '%s' "$FIXED_CRF_SVT_VINTAGE"; return; }
      [ "$tv" = true ]    && { printf '%s' "$FIXED_CRF_SVT_TV"; return; }
      printf '%s' "$FIXED_CRF_SVT_MOVIE" ;;
    hevc)
      [ "$hdr" = true ]   && { printf '%s' "$FIXED_CRF_X265_HDR"; return; }
      [ "$anime" = true ] && { printf '%s' "$FIXED_CRF_X265_ANIME"; return; }
      [ "$wanime" = true ] && { printf '%s' "$FIXED_CRF_X265_WANIME"; return; }
      [ "$vintage" = true ] && { printf '%s' "$FIXED_CRF_X265_VINTAGE"; return; }
      [ "$tv" = true ]    && { printf '%s' "$FIXED_CRF_X265_TV"; return; }
...
_vmaf_score_one() {  # clip crf codec model anime wanime vintage
  local clip="$1" crf="$2" codec="$3" model="$4"
  local anime="${5:-false}" wanime="${6:-false}" vintage="${7:-false}"
  local out vlog v svtp x265p extras
  local -a grain_decode_flag=()
  out="${clip%.mkv}-enc-$codec-$crf.mkv"
  vlog="${out%.mkv}.vmaf.json"
  case "$codec" in
    av1)
      svtp="enable-qm=1:qm-min=0"
      extras="$(svtav1_profile_extras "$anime" "$wanime" "$vintage")"
      [ -n "$extras" ] && svtp="$svtp:$extras"
      ...
      if svtav1_profile_uses_grain_synthesis "$anime" "$vintage"; then
        grain_decode_flag=(-export_side_data film_grain)
      fi
      ;;
    hevc)
      x265p="log-level=error"
      if [ "$anime" = true ]; then
        x265p="log-level=error:tune=animation"
      elif [ "$vintage" = true ]; then
        x265p="log-level=error:tune=grain"
      fi
...
vmaf_crf_search_internal() {  # src codec target model anime wanime vintage
  ...
  _probe_crf() {  # crf -> populates score/bytes; totals across all samples
    ...
    for c in "${clips[@]}"; do
      r="$(_vmaf_score_one "$c" "$crf" "$codec" "$model" "$anime" "$wanime" "$vintage")" || return 1
      v="${r%% *}"; b="${r##* }"
...
vmaf_crf_search_abav1() {  # src codec target model wanime
  local src="$1" codec="$2" target="$3" model="$4" wanime="${5:-false}"
  local enc preset out line crf vmaf pred extras kv p
  local -a svt_args=()
  case "$codec" in
    av1)
      enc="libsvtav1"; preset="$SVT_PRESET_SEARCH"
      extras="$(svtav1_profile_extras false "$wanime" false)"
      if [ -n "$extras" ]; then
        IFS=':' read -ra kv <<<"$extras"
        for p in "${kv[@]}"; do svt_args+=(--svt "$p"); done
      fi
      ;;
    hevc) enc="libx265";   preset="medium" ;;
  esac
  out="$("$AB_AV1_BIN" crf-search -i "$src" -e "$enc" --preset "$preset" "${svt_args[@]}" \
...
resolve_crf_for_encode() {  # src codec anime hdr
  ...
  local tv=false wanime=false vintage=false
  if [ "$anime" != true ] && uses_tv_profile "$src"; then
    tv=true
  fi
  if uses_wanime_profile; then
    wanime=true
  fi
  if uses_vintage_profile; then
    vintage=true
  fi
  ...
  if [ "$hdr" = true ] || [ "$VMAF_DISABLED" = true ] || [ "$FF_HAS_LIBVMAF" != true ]; then
    crf="$(fixed_crf_for "$codec" "$anime" "$tv" "$wanime" "$vintage" "$hdr")"
    [ "$hdr" = true ] && log_err "HDR source — fixed CRF $crf (VMAF unreliable on PQ/HLG)" \
                      || log_err "Fixed CRF $crf ($codec$( [ "$anime" = true ] && printf ', anime')$( [ "$wanime" = true ] && printf ', wanime')$( [ "$vintage" = true ] && printf ', vintage')$( [ "$tv" = true ] && printf ', tv'))"
  ...
  # Grain-synthesizing AV1 profiles (anime, vintage) cannot use ab-av1: its
  ...
  if [ -n "$AB_AV1_BIN" ] && { [ "$codec" != av1 ] || ! svtav1_profile_uses_grain_synthesis "$anime" "$vintage"; }; then
    res="$(vmaf_crf_search_abav1 "$src" "$codec" "$target" "$model" "$wanime")" || res=""
  fi
  if [ -z "${res:-}" ]; then
    res="$(vmaf_crf_search_internal "$src" "$codec" "$target" "$model" "$anime" "$wanime" "$vintage")" || res=""
  fi
  if [ -z "$res" ]; then
    crf="$(fixed_crf_for "$codec" "$anime" "$tv" "$wanime" "$vintage" "$hdr")"
...
ffmpeg_encode() {
  ...
  local anime=false tv=false wanime=false vintage=false hdr=false hdr_mode crf rc acodec abr
  ...
  rc=0
  build_ffmpeg_video_args "$codec" "$crf" "$src" "$anime" "$hdr" "$hdr_mode" "$wanime" "$vintage" || rc=$?
  if [ "$rc" -eq 2 ]; then
  ...
  log "ffmpeg encode ($codec crf=$crf$( [ "$hdr" = true ] && printf ', HDR10')$( [ "$anime" = true ] && printf ', anime')$( [ "$wanime" = true ] && printf ', wanime')$( [ "$vintage" = true ] && printf ', vintage')$( [ "$tv" = true ] && printf ', tv')): $(basename "$src")"
  run_ffmpeg "${args[@]}"
  rc=$?
  if [ "$rc" -ne 0 ]; then
...
remux_copy_to_mkv() {
  ...
  finalize_mkv_output "$real_dst" "$src"
}
